var soft = {
      ...
      columns: [
        {
          ...
          templet: function (item) {
            ...
            if (webcache && is_php) {
              ...
              webTypeCache == 'ols' &&
              distribution.indexOf('centos') == -1 &&
              item.name.indexOf('php-8.1') >= 0
            ) {
            ...
            return (
              ...
              ' ' +
              sStyle +
              ' ><img class="sort_img_err" alt="" ' +
              (item.type === 10 ? 'style="height:20px;width:22px"' : '') +
              ' src="/static/img/soft_ico/ico-' +
              fName +
              '.png">' +
              (!is_php || !item.version ? item.title : 'PHP-' + item.version) +
              ' ' +
              version +
              (item.is_beta ? '-<span style="color:#FC6D26">[测试版]</span>' : '') +
              '</span>'
            )
          },
        },
          ...
          templet: function (item) {
            ...
            if (item.price > 0) {
              price =
                ...
                (item.price / (item.name === 'btiplibrary' ? 365 : 30)).toFixed(2) +
                '元</span>'
            }
            return price
          },
              ...
              templet: function (item) {
                return item.sort !== undefined
                  ? '<a href="javascript:;" onclick="score.open_score_view(' +
                      ...
                      (item.sort <= 0 || item.sort > 5 ? '无评分' : item.sort.toFixed(1)) +
                      '</a>'
                  : '--'
              },
            }
            ...
            if (item.pid > 0) {
              ...
              if (item.endtime > 0) {
                if (item.type != 10) {
                  //判断是否为第三方插件
                  endtime =
                    bt.format_data(item.endtime, 'yyyy/MM/dd') +
        ...
        },
        {
          ...
          templet: function (item) {
            ...
            if (item.endtime < 0 && item.pid > 0) {
              ...
              if (item.type != 10) {
                pay_opt +=
                  '<a class="btlink soft_hover" onclick=\'bt.soft.product_pay_view(' +
                  JSON.stringify({
                    name: item.title,
                    pid: item.pid,
                    type: item.type,
                ...
                pay_opt +=
                  '<a class="btlink soft_hover" onclick="bt.soft.re_plugin_pay_other(\'' +
                  item.title +
                  "','" +
                  item.pid +
                  "'," +
                ...
                if (is_php5) {
                  ...
                  if (item.setup && item.task == '1') {
                    if (pay_opt == '') {
                      ...
                      if (item.admin) {
                        option +=
                          '<a class="btlink soft_hover" onclick="bt.soft.set_lib_config(\'' +
                          item.name +
                          "','" +
                          item.title +
                          "','" +
                        ...
                        option +=
                          '<a class="btlink soft_hover" onclick="soft.set_soft_config(\'' +
                          item.name +
                          '\')">' +
                          lan.soft.setup +
                          '</a> | '
                    ...
                    option +=
                      '<a class="btlink soft_hover" onclick="bt.soft.un_install(\'' +
                      item.name +
                      '\')" >' +
                      lan.soft.uninstall +
                      '</a>'
                    ...
                    if (pay_opt) {
                      ...
                      option =
                        '<a class="btlink soft_hover" onclick="bt.soft.install(\'' +
                        item.name +
                        '\',this)"  >' +
                        lan.soft.install +
                        '</a>'
              ...
              if (rdata.apache22 && is_php && $.inArray(item.name, phps) == -1) {
                ...
                if (item.setup && item.task == '1') {
                  if (pay_opt == '') {
                    ...
                    if (item.versions.length > 1) {
                      ...
                      if (item.admin === true) {
                        ...
                        if (is_update)
                          option +=
                            '<a class="btlink soft_hover" onclick="bt.soft.update_soft(\'' +
                            item.name +
                            "','" +
                            item.title +
                            "','" +
                            up_version.m_version +
                            "','" +
                            up_version.version +
                            "','" +
                            up_version.update_msg.replace(/\n/g, '_bt_') +
                            "','" +
                            item.type +
                            '\')" >更新</a> | '
                    ...
                    if (item.type !== 5 && item.type !== 10)
                      option +=
                        '<a href="javascript:;" class="btlink soft_hover" onclick="bt.soft.plugin_toolbox_info(\'' +
                        item.name +
                        "','" +
                        item.title +
                        "','" +
                  ...
                  if (pay_opt) {
                    ...
                    if (typeof item.preview_url != 'undefined' && item.preview_url != '') {
                      option =
                        '<a class="btlink soft_hover" href="' + item.preview_url + '" target="_blank">预览</a> | '
                    } else {
                      option = ''
                    }
                    option +=
  ...
  set_soft_tips: function (rdata, type) {
    ...
    var remarks = rdata.remarks,
      tips_info = $(
        '<div class="alert ltd" style="margin-bottom:0"><div class="soft_tips_text"></div><div class="btn-ground" style="display:inline-block;"></div><div class="close_recommend hide"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span><span>关闭推荐</span></div></div>'
      ),
    ...
    if (type === 10) {
      ...
      $.each(remarks.wx_list, function (index, wx_item) {
        wx_li +=
          '<div class="kf_fixed">\
            ...
            <span id="wx_item" class="btlink" style="font-size:13px;">' +
          wx_item.ps +
          '</span>\
            <div class="wx_kf_mask"><p>请打开微信"扫一扫"</p><div id="wx_qcode"><img src="/static/images/customer-qrcode.png" /></div><i class="wechatEnterprise"></i><p>【' +
          wx_item.ps +
          '】</p></div>\
        </div>'
      })
      explain.html(
        '<div class="pro_introduce_content">' +
          '<div class="pro_ic_title">' +
          '<span class="advantage"></span>' +
          '<span class="renew_info">' +
          '<span class="maturity_time" onclick="' +
          (genre ? 'bt.soft.updata_pro(33)' : 'bt.soft.updata_ltd(undefined,33)') +
          '"style="cursor: pointer;"' +
          (is_buy ? (countTime <= 7 && countTime >= 0 ? 'color:red;"' : '"') : '') +
          '>' +
          (is_buy
            ? '<span>到期时间：</span>' +
              (function () {
      ...
      var btn = $(
        '<a title="' +
          ('立即' + (is_buy ? '续费' : '购买')) +
          '" href="javascript:;" class="btn btn-success btn-xs va0" style="margin-left:10px;">' +
          ('立即' + (is_buy ? '续费' : '购买')) +
          '</a>'
      )
      ...
      for (var i = 0; i < rdata.list.length; i++) {
        ...
        if (rdata.list[i].id === 0) {
          remove_opt =
            ...
            rdata.list[i].name +
            '\')">更新</a> | <a class="btlink" onclick="soft.remove_other_dep(\'' +
            rdata.list[i].name +
            '\')">删除</a>'
        ...
        zbody +=
          '<tr>' +
          '<td><img style="width:22px;height:20px" src="' +
          ...
          rdata.list[i].php +
          '</td>' +
          '<td><a class="btlink" target="_blank" rel="noreferrer noopener" href="' +
          ...
          (rdata.list[i].sort !== undefined
            ? '<a href="javascript:;" class="btlink open_score_view" onclick="score.open_score_view(' +
              ...
              rdata.list[i].count +
              ')" >' +
              (rdata.list[i].sort <= 0 || rdata.list[i].sort > 5 ? '无评分' : rdata.list[i].sort.toFixed(1)) +
              '</a>'
            : '--') +
          '</td>' +
    ...
    $.post(
      ...
      function (rdata) {
        var con =
          ...
          rdata.name +
          '" name="name"  placeholder="项目英文名" style="width:190px" />\
					    <span class="c9" style="margin-left: 5px;">格式: [0-9A-Za-z_-]+，不要带有空格和特殊字符</span>\
					</div>\
					<div class="line"><span class="tname">中文名</span>\
						<input class="bt-input-text" name="title" value="' +
          rdata.title +
          '" placeholder="项目中文名" style="width:190px" type="text">\
                        <span class="c9" style="margin-left: 5px;">用于显示到列表的名称</span>\
					</div>\
                    <div class="line"><span class="tname">PHP版本</span>\
						<input class="bt-input-text mr5 " name="php"  placeholder="如：53,54,55,56,70,71,72" style="width:190px" value="' +
          rdata.php +
          '" type="text" />\
						<span class="c9">多个请使用","(逗号)隔开，不要使用PHP5.2</span>\
					</div>\
          ...
          '" style="width:190px" placeholder="如：system,exec" type="text" />\
						<span class="c9">多个请使用","(逗号)隔开，只解禁必要函数</span>\
					</div>\
                    <div class="line"><span class="tname">项目版本</span>\
						<input class="bt-input-text mr5" name="version" value="' +
          rdata.version +
          '" style="width:190px" placeholder="如：5.2.1" type="text" />\
						<span class="c9">当前导入的项目版本</span>\
					</div>\
                    <div class="line"><span class="tname">简介</span>\
  ...
  set_soft_config: function (name) {
    ...
    bt.soft.get_soft_find(name, function (rdata) {
      ...
      setTimeout(function () {
        menu.append(
          $('<p class="bgw bt_server" onclick="soft.get_tab_contents(\'service\',this)">' + lan.soft.service + '</p>')
        )
          ...
          for (var i = 0; i < phpSort.length; i++) {
            ...
            if (item) {
              if (item.os == undefined || item['os'] == bt.os) {
                ...
                if (
                  webcache &&
                  (item.type == 'set_fpm_config' || item.type == 'fpm_config' || item.type == 'get_php_status')
                )
                  continue
    ...
    bt_tools.send(
      {
        ...
        data: { name: 'site_push', data: JSON.stringify(params), id: Date.now() },
      },
      function (res) {
  ...
  get_tab_contents: function (
    ...
    switch (key) {
        ...
        bt.pub.get_ftp_logs(function (_status) {
          ...
          if (ltd < 0) {
            tabCon.append(
              ...
              <div class="prompt_description" style="margin-top: -60px;"><i class="layui-layer-ico layui-layer-ico0" style="width: 20px;height: 20px;display: inline-block;margin-right: 15px;vertical-align: middle;background-size: 700%;"></i>此功能为企业版专享， <a class="btlink" onclick="bt.soft.product_pay_view({totalNum:56,limit:\'' +
                'ltd' +
                '\',closePro:false})">立即购买</a></div></div>'
            )
          }
        })
        break
      case 'service': // nginx服务
        ...
        if (data.name == 'phpmyadmin') {
          ...
          if (data.name == 'nginx' || data.name == 'apache' || data.name == 'mysql' || data.name == 'pureftpd') {
            ...
            bt_tools.send({ url: '/push?action=get_push_list' }, function (ress) {
              ...
              $('#alert_setting').html(
                '<span style="margin-right:10px;font-size:14px">状态停止时提醒我:</span><div class="outer_state_alert"><input class="btswitch btswitch-ios" type="checkbox" ' +
                  (alert_message.status ? 'checked' : '') +
                  '><label class="btswitch-btn"></label></div> <span class="btlink setting_alert" style="margin-left:10px;font-size:14px">告警设置</span>'
              )
              // 打开详细设置弹窗
              $('.setting_alert').click(function () {
                bt_tools.open({
                        ...
                        group: {
                          ...
                          boxcontent:
                            '<div class="mr10 alert_status_input"  style="vertical-align: middle;display: inline-block;">\
														<input class="btswitch btswitch-ios" type="checkbox" ' +
                            (alert_message.status ? 'checked' : '') +
                            '>\
                        ...
                        for (var key in rdata) {
                          ...
                          if (key === 'wx_account') {
                            ...
                            wx_html =
                              ...
                              (alert_message.module.indexOf(item.name) > -1
                                ? !item.setup || $.isEmptyObject(item.data)
                                  ? ''
                                  : key == 'wx_account' && !accountConfigStatus
                                  ? ''
                              ...
                              (!item.setup || $.isEmptyObject(item.data)
                                ? '[<a target="_blank" class="bterror installNotice" data-type="' +
                                  ...
                                  '">点击安装</a>]'
                                : key == 'wx_account' && !accountConfigStatus
                                ? '[<a target="_blank" class="bterror alertInstall" data-type="' +
                                  item.name +
                                  '">未配置</a>]'
                                : '') +
                              '</span>' +
                  ...
                  yes: function (form, index) {
                    ...
                    // 判定间隔时间不可为负数
                    if (
                      Number(form.interval) < 60 ||
                      Number(form.interval) === '' ||
                      !bt.isInteger(parseFloat(form.interval))
                    )
                      return layer.msg('间隔时间范围应为大于60的正整数', { icon: 2, time: 2000 })
                    var params = {
                      type: 'services',
                ...
                if (!status) {
                  ...
                  if (alert_message.module === '' || !is_installFlag) {
                    ...
                    $.each(soft.alert_modules, function (index) {
                      // 安装了该模块且有配置信息
                      if (
                        Object.keys(soft.alert_modules[index].data).length &&
                        soft.alert_modules[index].name !== 'sms'
                      ) {
        ...
        if (data.name == 'phpmyadmin') {
          tabCon.append(
            '<div style="margin-top:25px;">\
              ...
              '\')" style="vertical-align: top;margin-right: 10px;"><label class="mr20" for="pma_status" style="font-weight:normal;vertical-align: sub;">启用公共访问权限</label></div>\
                    <p style="margin-top:5px;"><span>公共访问地址: </span><a class="btlink" href="' +
              data.ext.url +
              '" target="_blank">' +
              data.ext.url +
              '</a></p>\
        ...
        var help =
          '<ul class="help-info-text c7 mtb15" style="padding-top:30px"><li>' + lan.soft.mysql_mem_err + '</li></ul>'
        ...
        if (data.name.indexOf('php-') >= 0) {
          ...
          bt.soft.get_soft_find('security_notice', function (rdata_res) {
            if (rdata_res.endtime > 0) {
              ...
              bt_tools.send(
                ...
                function (global_config) {
                  ...
                  bt_tools.send(
                    ...
                    function (res) {
                      var protective_switch = res.php_versions
                        ? res.php_versions.filter(function (item) {
                      ...
                      tabCon.append(
                        '<div class="php_pro">\
                          ...
                          (protective_switch.state ? 'checked' : '') +
                          '>\
                          ...
                          ((!global_config.send || !global_config.status) && protective_switch.state
                            ? '<div class="sn-home--open-condition"><i class="sn-home--important-note">!</i>未启用PHP安全告警，请先' +
                              ...
                              (!global_config.status && !global_config.send ? '、' : '') +
                              (global_config.status ? '' : '开启<span class="set_status">全局监控开关</span>') +
                              '</div>'
                            : '') +
                          '</div><div class="php_site_table"></div>\
                        ' +
                          bt.render_help([
                              ...
                              function (alarms) {
                                ...
                                $.post('/plugin?action=a&name=security_notice&s=get_send_config', {}, function (rdata) {
                                  ...
                                  for (var i = 0; i < sendChannel.length; i++) {
                                    ...
                                    var is_config =
                                      sendChannel[i].value !== 'NONE' &&
                                      (!alarms[sendChannel[i].value]['setup'] ||
                                        $.isEmptyObject(alarms[sendChannel[i].value]['data']))
                                        ? true
                                        : false
                                    html +=
                        ...
                        bt.simple_confirm(
                          {
                            ...
                            msg: _status
                              ? '关闭后，该版本的站点将不再受到安全防护，是否继续？'
                              : '开启后，该版本的站点将受到安全防护，是否继续？',
                          },
                          function () {
                            bt_tools.send(
                              {
                        ...
                        var siteTbale = bt_tools.table({
                          ...
                          column: [
                            { fid: 'site_name', title: '站点名称' },
                            {
                              ...
                              title: '防护状态',
                              type: 'switch',
                              event: function (row, index, ev, cname) {
                                ...
                                bt.confirm(
                                  {
                                    ...
                                    msg: _status
                                      ? '关闭后，该网站将不再接收安全告警，是否继续？'
                                      : '开启后，检测到安全问题时将会发送告警通知，是否继续？',
                                    cancel: function () {
                                      $(ev.currentTarget).prop('checked', row['open'] ? true : false)
                                    },
                                  },
                                  function () {
                                    ...
                                    $.post(
                                      {
                                        url:
                                          '/plugin?action=a&name=security_notice&s=' +
                                          (_status ? 'stop_site' : 'start_site'),
                                            ...
                                            column: [
                                              ...
                                              },
                                              {
                                                ...
                                                template: function (items) {
                                                  return (
                                                    '<span>' + (items.type === 'dir' ? '文件夹' : '文件') + '</span>'
                                                  )
                                                },
                                              },
                                              {
                                            ...
                                            bt_tools.open({
                                              ...
                                              success: function (layero, index) {
                                                ...
                                                $('.module-check.check_disabled label')
                                                  .unbind('click')
                                                  .click(function () {
                                                    if ($(this).parent().hasClass('check_disabled'))
                                                      layer.tips('目录暂不支持该告警操作！', $(this), {
                                                        tips: [3, '#ff0000'],
                                                        time: 3000,
                                                      })
                                                  })
                                              ...
                                              yes: function (form, layers, indexs) {
                                                ...
                                                if (actions.join(',') === '0,0,0')
                                                  return layer.msg('最少选择一个告警操作！')
                                                //actions: 1,1,1,1 读取 删除 修改 增加
                                                bt_tools.send(
                                                  {
                                                    url:
                                                      '/plugin?action=a&name=security_notice&s=' +
                                                      (rowdata ? 'edit_site_config' : 'add_site_config'),
                                                    data: {
                                                      domain: row.site_name,
                                                      path: form.path,
                                              ...
                                              column: [
                                                ...
                                                },
                                                {
                                                  ...
                                                  template: function (row) {
                                                    ...
                                                    if (dpath.length > 35) {
                                                      dpath =
                                                        dpath.substring(0, 20) +
                                                        '...' +
                                                        dpath.substring(dpath.length - 10, dpath.length)
                                                    }
                                                        ...
                                                        layer.open({
                                                          ...
                                                          content:
                                                            ...
                                                            '</div>\
                                                                ...
                                                                <div class="lib-con mt10">\
                                                                    <div class="divpre">' +
                                                            row['data_info']['data']['request']['headers'][
                                                              'user-agent'
                                                            ] +
                                                            '</div>\
                                                                </div>\
                                                        ...
                                                        if (_http_info) {
                                                          layer.open({
                                                            ...
                                                            success: function (layers) {
                                                              $('#http_info_data').text(
                                                                bt.formatJsonForNotes(_http_info)
                                                              )
                                                              $(layers).css(
                                                                'top',
                                                                ($(window).height() - $(layers).height()) / 2
                                                              )
                                                            },
                                                          })
                                                        } else {
              ...
              tabCon.append(
                '<div class="webedit-con-plugin"><div class="thumbnail-introduce-new">\
                      <div class="thumbnail-introduce-title-new">\
                        ...
                        <div class="thumbnail-title-button-new daily-product-buy"><button type="button" class="btn btn-success va0 buyIntrusion" onclick=\'bt.soft.product_pay_view(' +
                  JSON.stringify({
              ...
              $('.img_view img,.img_view_mask').click(function () {
                layer.open({
                  ...
                  btn: false,
                  content:
                    '<img src="https://www.bt.cn/Public/new/plugin/introduce/soft/securityNotice.png" style="width:100%" />',
                })
              })
              ...
              $('.thumbnail-box .thumbnail-item')
                ...
                .on('click', function () {
                  ...
                  bt.open({
                    ...
                    area: ['900px', '680px'],
                    content:
                      '<img src="/static/images/recommend/securityNotice' +
                      (index ? index + 1 : '') +
                      '.png" style="width:100%;height: 680px;"/>',
      ...
      case 'config':
        ...
        tabCon.append(
          '<div class="bt-input-text ace_config_editor_scroll" style="height: 320px;min-height:350px;line-height:18px;" id="textBody"></div>'
        )
        tabCon.append(
      ...
      case 'get_mysql_run_status':
        bt.send('GetRunStatus', 'database/GetRunStatus', {}, function (rdata) {
          var cache_size =
            (
              (parseInt(rdata.Qcache_hits) / (parseInt(rdata.Qcache_hits) + parseInt(rdata.Qcache_inserts))) *
              100
            ).toFixed(2) + '%'
          ...
          var Con =
            '<div class="divtable"><table class="table table-hover table-bordered" style="width: 650px;margin-bottom:10px;background-color:#fafafa">\
								<tbody>\
									<tr><th>' +
      ...
      case 'mysql_error_log':
        ...
        bt.send(
          ...
          function (rdata) {
            ...
            var limitCon =
              '<p class="conf_p">\
                        <button class="btn btn-default btn-clear btn-xss" style="float:left;" >' +
            ...
            $('.btn-clear').click(function () {
              bt.simple_confirm(
                {
                  title: '清空日志操作提示',
                  msg: '清空日志会导致，运行错误信息丢失，无法排除问题，请谨慎操作，是否继续？',
                },
                function (index) {
      ...
      case 'mysql_slow_log':
        ...
        bt.send('GetSlowLogs', 'database/GetSlowLogs', {}, function (logs) {
          ...
          var phpCon =
            '<textarea readonly="" style="margin: 0px;width: 650px;height: 552px;background-color: #333;color:#fff; padding:0 5px" id="error_log">' +
            logs.msg +
            '</textarea>'
      ...
      case 'mysql_log_bin':
        ...
        var inHtml =
          '<p class="conf_p">\
                        <span class="f14 c6" style="width:70px">' +
        ...
        $('#btn-bin').click(function () {
          if (!$('#btn-bin').prop('checked')) {
            ...
            bt.simple_confirm(
              { title: '二进制日志开关操作', msg: '关闭二进制日志后，将不会再记录所有对数据库的修改操作' },
              function (index) {
                bt_tools.send(
                  { url: '/database?action=BinLog' },
                  function (datas) {
            ...
            bt.simple_confirm(
              {
                title: '二进制日志开关操作',
                msg: '开启二进制日志可以保护Mysql的数据安全，但同时会消耗一定磁盘存储，是否继续?',
              },
              function (index) {
                var loadC = layer.msg('正在设置中,请稍后...', {
                  icon: 16,
      ...
      case 'log':
        ...
        bt.send(
          ...
          function (rdata) {
            ...
            var ebody =
              '<div class="soft-man-con"><textarea readonly="" style="margin: 0px;width: 500px;height: 520px;background-color: #333;color:#fff; padding:0 5px" id="error_log">' +
              rdata.msg +
              '</textarea></div>'
      ...
      case 'memcached_set':
        ...
        bt.send('GetMemcachedStatus', 'ajax/GetMemcachedStatus', {}, function (rdata) {
          ...
          if (key == 'memcached_set') {
                ...
                items: [
                  {
                    ...
                    callback: function (ldata) {
                      ...
                      if (!bt.check_port(ldata.port)) {
                        layer.msg(
                          '端口格式错误，可用范围：1-65535<br />请避免使用以下端口【22,80,443,8080,8443,8888】',
                          {
                            icon: 2,
                          }
                        )
            ...
            var con =
              '<div class="divtable"><table id=\'tab_memcached_status\' style="width: 490px;" class=\'table table-hover table-bordered \'><thead><th>字段</th><th>当前值</th><th>说明</th></thead></table></div>'
            $('.soft-man-con').html(con)
      ...
      case 'get_phpinfo':
        ...
        $.post(
          ...
          function (php_info) {
            ...
            con +=
              '<tr><td style="width:70px">PHP版本</td><td>' +
      ...
      case 'set_dis_fun':
        bt.soft.php.get_config(version, function (rdata) {
          ...
          _bt_form.append(
            "<div class='divtable mtb15' style='height:350px;overflow:auto'><table id=\"blacktable\" class='table table-hover' width='100%' style='margin-bottom:0'></table><div>"
          )
          var _tab = bt.render({
            table: '#blacktable',
            data: list,
      ...
      case 'get_php_status':
        bt.soft.php.get_php_status(version, function (rdata) {
          ...
          var con =
            "<div style='height:450px;overflow:auto;'><table id='tab_php_status' class='table table-hover table-bordered' style='margin:0;padding:0'></table></div>"
          $('.soft-man-con').html(con)
        ...
        bt.soft.php.get_php_session(version, function (res) {
          $('.soft-man-con').html(
            '<div class="conf_p">' +
              ...
              '>files</option>' +
              (version != '52'
                ? '<option value="redis" ' + (res.save_handler == 'redis' ? 'selected' : '') + '>redis</option>'
                : '') +
              (version != '73'
              ...
              '<input name="ip" class="bt-input-text mr10" type="text" style="width:160px" value="' +
              res.save_path +
              '">' +
              ...
              '<div class="info-r ">' +
              '<input name="port" class="bt-input-text mr5" type="text" style="width:160px" value="' +
              res.port +
              '">' +
              '</div>' +
              '</div>' +
              ...
              '<div class="info-r ">' +
              '<input name="passwd" class="bt-input-text mr5" placeholder="如果没有密码留空" type="text" style="width:160px" value="' +
              res.passwd +
              '">' +
              '</div>' +
              '</div>' +
          ...
          if (res.save_handler == 'files') {
            bt.soft.php.get_session_count(function (res) {
              $('.clear_conter').html(
                '<div class="session_clear_list"><div class="line"><span>总Session文件数量</span><span>' +
                  res.total +
                  '</span></div><div class="line"><span>可清理的Session文件数量</span><span>' +
                  res.oldfile +
                  '</span></div></div><button class="btn btn-success btn-sm clear_session_file">清理session文件</button>'
              )
      ...
      case 'get_fpm_logs':
        bt.soft.php.get_fpm_logs(version, function (logs) {
          var phpCon =
            '<textarea readonly="" style="margin: 0px;width: ' +
            (data.name.indexOf('php-') >= 0 ? '640' : '500') +
            'px;height: 560px;background-color: #333;color:#fff; padding:0 5px" id="error_log">' +
            logs.msg +
            '</textarea>'
          $('.soft-man-con').html(phpCon)
          var ob = document.getElementById('error_log')
      ...
      case 'get_redis_status':
        bt.soft.redis.get_redis_status(function (rdata) {
          var hit = (
            (parseInt(rdata.keyspace_hits) / (parseInt(rdata.keyspace_hits) + parseInt(rdata.keyspace_misses))) *
            100
          ).toFixed(2)
          var arrs = []
          ...
          var con =
            '<div class="divtable"><table id=\'tab_get_redis_status\' style="width: 490px;" class=\'table table-hover table-bordered \'><thead><th>字段</th><th>当前值</th><th>说明</th></thead></table></div>'
          $('.soft-man-con').html(con)
        ...
        var loadT = layer.open({
          ...
          content:
            ...
            '</a></p>\
                        ...
                        <div class="bt-form-submit-btn"><button type="button" class="btn btn-sm btn-danger mr5" onclick="layer.closeAll()">取消</button><button type="button" class="btn btn-sm btn-success" onclick="soft.input_zip(\'' +
            data.name +
            "','" +
            data.tmp_path +
            '\')">确定安装</button></div>\
...
bt_tools.send(
  ...
  function (ress) {
    ...
    $.each(ress[0].plugin, function (index, item) {
      $('#hotsearch').append(
        '<div style="font-size:12px;padding: 0 10px;" class="body_item" data-ename="' +
          item.ename +
          '">' +
          item.name +
          '</div>'
...
function AddSite(codename, title) {
  ...
  var data =
    ...
    '&webname=' +
    domain +
    '&ftp=false&sql=true&address=localhost&codeing=utf8mb4&version=' +
    php_version
  $.post('/site?action=AddSite', data, function (ret) {
    ...
    $.post('/deployment?action=SetupPackage', pdata, function (rdata) {
      ...
      sqlData +=
        ...
        (mainDomain + '/' + rdata.msg.success_url).replace('//', '/') +
...
function onekeyCodeSite(codename, versions, title, enable_functions) {
  $.post('/site?action=GetPHPVersion', function (rdata) {
    ...
    var con =
      ...
      default_path +
      '" placeholder="网站根目录" style="width:398px" type="text"><span class="glyphicon glyphicon-folder-open cursor" onclick="ChangePath(\'inputPath\')"></span> </div>\
					</div>\
...
var score = {
  ...
  render_score_info: function (obj, callback) {
    ...
    score.get_score_info(config, function (res) {
      ...
      $('.comment_rate').text(
        res.total !== 0 ? (((_split_score[0] + _split_score[1]) / res.total).toFixed(2) * 100).toFixed(0) + '%' : '0%'
      )
      for (var i = 0; i < 5; i++) {
        $('.comment_star_group:eq(' + i + ')')
          .find('.comment_progress .comment_progress_bgw')
          .css('width', ((_split_score[i] / res.total).toFixed(2) * 100).toFixed(0) + '%')
      }
      ...
      for (var j = 0; j < _data.length; j++) {
        _html +=
          ...
          (getLength(_data[j].ps) > 65
            ? reBytesStr(_data[j].ps, 65) + '...&nbsp;<a href="javascript:;" class="btlink">详情</a>'
            : _data[j].ps) +
          '</div>\
                </div>'
      }
      ...
      if (res.total > 10 && _data.length === 10) {
        _html +=
          '<div class="comment_box get_next_page"><span class="glyphicon glyphicon-chevron-down" aria-hidden="true"></span>点击获取更多评论</div>'
      }
