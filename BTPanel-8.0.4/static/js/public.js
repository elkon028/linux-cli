...
var aceEditor = {
  ...
  eventEditor: function () {
            ...
            success: function (layeo, index) {
              ...
              for (var i = 0; i < _item.historys.length; i++) {
                _html +=
                  ...
                  _item.historys[i] +
                  '">打开文件</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href="javascript:;" class="btlink recovery_file_historys" data-history="' +
                  _item.historys[i] +
                  '" data-path="' +
                  _item.path +
                  '">恢复</a></td></tr>'
      ...
      if ($(this).data('file') === 'Dir') {
        if (_menu.hasClass('glyphicon-menu-right')) {
          ...
          if ($(this).next().find('li').length === 0)
            _this.reader_file_dir_menu({ el: $(this).next(), path: _path, group: _group + 1 })
        } else {
          ...
          if (_width >= 265 && _width <= 450) {
            ...
            $('.file_fold .newly_file_input').width(
              $('.file_fold .newly_file_input').parent().parent().parent().width() -
                ($('.file_fold .newly_file_input').parent().parent().attr('data-group') * 15 - 5) -
                20 -
                30 -
                53
            )
    ...
    $('.ace_catalogue_list').on('click', '.has-children .edit_file_group .glyphicon-ok', function () {
      ...
      if (
        $(this).parent().parent().parent().attr('data-menu-path') === undefined &&
      ...
      if (_file_or_dir === '') {
        ...
        layer.tips(
          _type === 0 ? '文件目录不能为空' : _type === 1 ? '文件名称不能空' : '新名称不能为空',
          $(this).prev(),
  ...
  newly_file_type: function (that) {
    ...
    switch (_type) {
      ...
      case 6:
        ...
        layer.confirm(
          lan.get(is_files ? 'recycle_bin_confirm' : 'recycle_bin_confirm_dir', [
            _active.find('.file_title span').html(),
          ]),
  ...
  newly_file_type_dom: function (_active, _group, _type, _val) {
    ...
    _html +=
      '<i class="' + (_type == 0 ? 'folder' : _type == 1 ? 'text' : _this.get_file_suffix(_val || '')) + '-icon"></i>'
  ...
  get_file_dir_list: function (obj, callback) {
    var loadT = layer.msg('正在获取文件列表，请稍候...', { time: 0, icon: 16, shade: [0.3, '#000'] }),
      _this = this
    ...
    $.post(
      ...
      { p: obj.p, showRow: obj.showRow, sort: obj.sort, reverse: obj.reverse, path: obj.path, search: obj.search },
      function (res) {
        layer.close(loadT)
        if (callback) callback(res)
      }
  ...
  setEncodingType: function (encode) {
    ...
    $('.menu-encoding ul li').click(function (e) {
      ...
      $(this)
        .addClass('active')
        .append('<span class="icon"><i class="glyphicon glyphicon-ok" aria-hidden="true"></i></span>')
        .siblings()
        .removeClass('active')
        .find('span')
        .remove()
  ...
  currentStatusBar: function (id) {
    ...
    $('.ace_conter_toolbar [data-type="history"]').html(
      '历史版本：<i>' + (_item.historys.length === 0 ? '无' : _item.historys.length + '份') + '</i>'
    )
    $('.ace_conter_toolbar [data-type="path"]').html('文件位置：<i title="' + _item.path + '">' + _item.path + '</i>')
    $('.ace_conter_toolbar [data-type="tab"]').html(
      _item.softTabs ? '空格：<i>' + _item.tabSize + '</i>' : '制表符长度：<i>' + _item.tabSize + '</i>'
    )
  ...
  creationEditor: function (obj, callabck) {
    ...
    ACE.ace.getSession().selection.on('changeCursor', function (e) {
      ...
      $('[data-type="cursor"]').html(
        '行<i class="cursor-row">' + (_cursor.row + 1) + '</i>,列<i class="cursor-line">' + _cursor.column + '</i>'
      )
    })
  ...
  saveFileMethod: function (ACE, force, callabck) {
    ...
    layer.msg('保存文件中，请稍候<img src="/static/img/ns-loading.gif" style="width:15px;margin-left:5px">', {
      icon: 0,
    })
    _this.saveFileBody(
      {
...
$('#memu_custom .memu_custom').hover(
  function () {
    if (!$('.memu_hint').length)
      layer.tips('<span class="memu_hint">点击设置左侧菜单栏隐藏</span>', '#memu_custom .memu_custom', {
        tips: [2, '#2c3138'],
        time: 0,
      })
  },
...
function getFlieSpeed(sele, wsStr, mod_name, rdata, fun) {
  if (!$(sele).length) return
  body =
    '<p class="bt-progress-search"><span>文件搜索中<span><img src="/static/img/ing.gif"></p>\
		<div class="bt-progress"><div class="bt-progress-bar" style="width:0%"><div class="bt-progress-text">0%</div></div></div>\
		<p class="f12 c9"><span class="pull-left bt-progress-used">0</span>' +
    '/' +
    '<span class="bt-progress-total">0</span></p>'
  $(sele).prev().hide()
  $(sele).css({ 'margin-left': '-37px', width: '380px' })
  $(sele).parents('.layui-layer').css({ 'margin-left': '-100px' })
  $(sele).html(body)
  ...
  send()
  wsFiles.addEventListener('open', function () {
    var _html = '<span>文件搜索中<span><img src="/static/img/ing.gif">'
    $(sele + ' .bt-progress-search').html(_html)
  })
  wsFiles.addEventListener('message', function (res) {
    var msg = JSON.parse(res.data)
    if (!msg.hasOwnProperty('result')) {
      $('.bt-progress-used').text(msg.num)
      $('.bt-progress-total').text(msg.total)
      if (msg.progress < 100) {
        $('.my_file_speed .bt-progress-bar').css('width', msg.progress + '%')
        $('.my_file_speed .bt-progress-text').text(msg.progress + '%')
      }
    } else {
      fun(msg)
    }
...
function reader_realtime_tasks(refresh) {
  get_realtime_tasks(function (res) {
    ...
    if (typeof res.task === 'undefined') {
      html =
        '<div style="padding:5px;height: 510px;">当前没有任务！</div><div style="padding-left: 5px;">若任务长时间未执行，请尝试在首页点【重启面板】来重置任务队列</div>'
...
function to_login() {
  layer.confirm(
    '您的登陆状态已过期，请重新登陆!',
    { title: '会话已过期', icon: 2, closeBtn: 1, shift: 5 },
    function () {
      location.reload()
    }
  )
...
var Term = {
  ...
  on_message: function (ws_event) {
    ...
    if (
      (result.indexOf('@127.0.0.1:') != -1 || result.indexOf('@localhost:') != -1) &&
...
function web_shell(ssh_info, is_max) {
  Term.run(ssh_info, is_max)
}
              ...
              if (res) {
                ...
                for (var item = 0; item < res_data.length; item++) {
                  if (res_data[item].level == 0) {
                    ...
                    $('#public_address').text(
                      res_data[item].country + ' ' + res_data[item].province + ' ' + res_data[item].city
                    )
                    continue
                  }
                ...
                layer.confirm(
                  ...
                  function () {
                    bt.send(
                      'Set_bt_host',
                      'ajax/Set_bt_host',
                      { ip: $('.node_select').find('.active').find('.noed_item_radio').data('ip') },
                      function (result) {
...
var product_recommend = {
          ...
          if (typeof html === 'string') {
            layer.open({
              ...
              success: function () {
                ...
                if (!is_event)
                  new Function(item.eventList[item.eventList.length - 1].event.replace('$siteName', param))()
              },
            })
          }
        })
  ...
  pay_product_sign: function (type, source, plugin) {
    return false
    if (typeof plugin != 'undefined' && plugin == 'ltd')
      return bt.soft.product_pay_view({ totalNum: source, limit: 'ltd', closePro: true })
    switch (type) {
      case 'pro':
      ...
      if (res.status) {
        ...
        if (item.version !== item.info.version) {
          updateInfo =
            '【' +
            item['title'] +
            '】模块存在新的版本，为了不影响使用，请更新。<button class="btn btn-success btn-sm btn-update">立即更新</button>'
        }
      ...
      list: [
        {
          ...
          success: function () {
            ...
            $('.deteles').hover(
              function () {
                $(this).css({
                  border: '1px solid rgba(239, 8, 8, 0.3)',
                  background: 'rgba(239, 8, 8, 0.06)',
                  color: '#EF0808',
                })
              },
              function () {
            ...
            var file_bin_table = bt_tools.table({
              ...
              beforeRequest: function (p) {
                ...
                return {
                  ...
                  p: p.p,
                  limit: p.limit,
                  search: $('.search_input').val() !== '' ? $('.search_input').val() : '',
              ...
              column: [
                { type: 'checkbox', class: '', width: 20 },
                {
                  ...
                  template: function (row) {
                    ...
                    var imgSrc =
                      row.name.indexOf('.log') > -1
                        ? '/static/img/file_icon/log.png'
                        : row.name.indexOf('.html') > -1
                        ? '/static/img/file_icon/html.png'
                        : row.name.indexOf('.sh') > -1
                        ? '/static/img/file_icon/shell.png'
                        : row.name.indexOf('.7z') > -1
                        ? '/static/img/file_icon/7z.png'
                        : row.name.indexOf('.access') > -1
                        ? '/static/img/file_icon/access.png'
                        : row.name.indexOf('.apk') > -1
                        ? '/static/img/file_icon/apk.png'
                        : row.name.indexOf('.avi') > -1
                        ? '/static/img/file_icon/avi.png'
                        : row.name.indexOf('.bmp') > -1
                        ? '/static/img/file_icon/bmp.png'
                        : row.name.indexOf('.cdr') > -1
                        ? '/static/img/file_icon/cdr.png'
                        : row.name.indexOf('.css') > -1
                        ? '/static/img/file_icon/css.png'
                        : row.name.indexOf('.doc') > -1
                        ? '/static/img/file_icon/doc.png'
                        : row.name.indexOf('.compress') > -1
                        ? '/static/img/file_icon/compress.png'
                        : row.name.indexOf('.gif') > -1
                        ? '/static/img/file_icon/gif.png'
                        : row.name.indexOf('.ico') > -1
                        ? '/static/img/file_icon/ico.png'
                        : row.name.indexOf('.java') > -1
                        ? '/static/img/file_icon/java.png'
                        : row.name.indexOf('.js') > -1
                        ? '/static/img/file_icon/javascript.png'
                        : row.name.indexOf('.json') > -1
                        ? '/static/img/file_icon/json.png'
                        : row.name.indexOf('.lua') > -1
                        ? '/static/img/file_icon/lua.png'
                        : row.name.indexOf('.mkv') > -1
                        ? '/static/img/file_icon/mkv.png'
                        : row.name.indexOf('.mov') > -1
                        ? '/static/img/file_icon/mov.png'
                        : row.name.indexOf('.mp4') > -1
                        ? '/static/img/file_icon/mp4.png'
                        : row.name.indexOf('.mpeg') > -1
                        ? '/static/img/file_icon/mpeg.png'
                        : row.name.indexOf('.mpg') > -1
                        ? '/static/img/file_icon/mpg.png'
                        : row.name.indexOf('.pdf') > -1
                        ? '/static/img/file_icon/pdf.png'
                        : row.name.indexOf('.php') > -1
                        ? '/static/img/file_icon/php.png'
                        : row.name.indexOf('.ppt') > -1
                        ? '/static/img/file_icon/ppt.png'
                        : row.name.indexOf('.py') > -1
                        ? '/static/img/file_icon/py.png'
                        : row.name.indexOf('.rar') > -1
                        ? '/static/img/file_icon/rar.png'
                        : row.name.indexOf('.rm') > -1
                        ? '/static/img/file_icon/rm.png'
                        : row.name.indexOf('.rmvb') > -1
                        ? '/static/img/file_icon/rmvb.png'
                        : row.name.indexOf('.sql') > -1
                        ? '/static/img/file_icon/sql.png'
                        : row.name.indexOf('.swf') > -1
                        ? '/static/img/file_icon/swf.png'
                        : row.name.indexOf('.tar') > -1
                        ? '/static/img/file_icon/tar.png'
                        : row.name.indexOf('.webm') > -1
                        ? '/static/img/file_icon/webm.png'
                        : row.name.indexOf('.wma') > -1
                        ? '/static/img/file_icon/wma.png'
                        : row.name.indexOf('.wmv') > -1
                        ? '/static/img/file_icon/wmv.png'
                        : row.name.indexOf('.xls') > -1
                        ? '/static/img/file_icon/xls.png'
                        : row.name.indexOf('.xml') > -1
                        ? '/static/img/file_icon/xml.png'
                        : row.name.indexOf('.zip') > -1
                        ? '/static/img/file_icon/zip.png'
                        : row.name.indexOf('.gif') > -1
                        ? '/static/img/file_icon/gif.png'
                        : row.name.indexOf('.ico') > -1
                        ? '/static/img/file_icon/ico.png'
                        : row.name.indexOf('.jpeg') > -1
                        ? '/static/img/file_icon/jpeg.png'
                        : row.name.indexOf('.jpg') > -1
                        ? '/static/img/file_icon/jpg.png'
                        : row.name.indexOf('.png') > -1
                        ? '/static/img/file_icon/png.png'
                        : row.name.indexOf('.webp') > -1
                        ? '/static/img/file_icon/webp.png'
                        : row.name.indexOf('.txt') > -1
                        ? '/static/img/file_icon/file.png'
                        : row.is_dir
                        ? '/static/img/file_icon/folder.png'
                        : '/static/img/file_icon/file.png'
                ...
                },
                {
                  ...
                  template: function (row) {
                    return (
                      '<span style="width:200px;display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' +
                      row.dname +
                      '</span>'
                    )
                  },
                    ...
                    },
                    {
                      ...
                      event: function (row, index, ev, key, that) {
                        ...
                        bt_tools.open({
                          ...
                          yes: function (indexs) {
                            var delete_load = layer.msg('正在恢复中，请稍后...', {
                              icon: 16,
                              time: 0,
                              shade: [0.3, '#000'],
                            })
                            bt_tools.send(
                              {
                                ...
                                data: {
                                  path: row.rname,
                                  rpath: type == 'left' ? row.dname : recoverForm.$get_form_value().path + row.name,
                                },
                              },
                              function (ress) {
                                layer.msg(ress.msg, {
                    ...
                    },
                    {
                      ...
                      event: function (row, index, ev, key, that) {
                        bt.input_confirm(
                          ...
                          function () {
                            var delete_load = layer.msg('正在删除中，请稍后...', {
                              icon: 16,
                              time: 0,
                              shade: [0.3, '#000'],
                            })
                            bt_tools.send(
                              { url: '/files?action=Del_Recycle_bin', data: { path: row.rname } },
                              function (ress) {
                                layer.msg(ress.msg, {
                                  icon: ress.status ? 1 : 2,
                                })
                  ...
                  selectList: [
                        ...
                        bt.input_confirm(
                          {
                            title: '批量删除文件',
                            value: '删除文件',
                            msg: '批量删除选中的文件后，<span class="color-org">文件将彻底删除，不可恢复</span>，是否继续操作？',
                          },
                          function () {
                    ...
                    },
                    {
                      ...
                      param: function (row) {
                        return {
                          path: row.rname,
                          rpath: isOriginalPath ? row.dname : recoverForm.$get_form_value().path + row.name,
                        }
                      },
                      callback: function (that) {
                        bt_tools.open({
                          ...
                          content:
                            '\
																<div style="margin:20px auto;width:270px;height:34px;background-color:rgba(240, 173, 78, 0.1);color:#F0AD4E;display:flex;flex-direction:row;align-items:center;margin-left:19px"><img src="/static/images/svg/tips.svg" style="margin-left:8px;margin-right:6px" />注意：关闭回收站，将永久删除的文件，不滞留在回收站</div>\
																<div style="display:flex;flex-direction:row;justify-content:start;align-items:center;margin-left:31px;margin-bottom:10px">\
																	<span> 恢复方法</span>\
																	<div style="width:158px;border-radius:5px;margin-left:15px">\
																	<button class="btn btn-sm btn-success bt-cp1-hover bt-cp Catalogue_show"\
																		id="Catalogue_show_left" style="margin-left:6px">原文件地址</button>\
																	<button style="border-radius:5px;margin-left:-4px" class="btn btn-sm btn-default bt-cp1-hover bt-cp Catalogue_show"\
																		id="Catalogue_show_right">其他目录</button>\
																	</div >\
																		</div>\
																<div id="recover"></div>\
            ...
            function data_reconstruction(data, type, callback) {
              ...
              $.each(data, function (index, item) {
                ...
                if (itemD[0].indexOf('.upload.tmp') != -1)
            ...
            function open_images_preview(data) {
              ...
              $('.preview_toolbar a').click(function () {
                ...
                switch (index) {
                  ...
                  case 3:
                    ...
                    images_config.scale =
                      (index == 2
                        ? Math.round((images_config.scale + 0.4) * 10)
                        : Math.round((images_config.scale - 0.4) * 10)) / 10
                    auto_images_size()
                    break
                  case 4:
              ...
              $('.preview_full,.preview_small').click(function () {
                if ($(this).hasClass('preview_full')) {
                  ...
                  mask
                    .css({ width: that.area[0], height: that.area[1], top: 0, left: 0, margin: 0 })
                    .data('type', 'full')
                  auto_images_size()
                } else {
              ...
              function auto_images_size(transition) {
                ...
                if (images_config.init_width > preview_width) {
                  ...
                  images_config.init_height = parseFloat(
                    ((preview_width / images_config.natural_width) * images_config.init_height).toFixed(2)
                  )
                }
                if (images_config.init_height > preview_height) {
                  images_config.init_width = parseFloat(
                    ((preview_height / images_config.natural_height) * images_config.init_width).toFixed(2)
                  )
                ...
                images_config.current_left = parseFloat(
                  ((images_config.preview_width - images_config.current_width) / 2).toFixed(2)
                )
                images_config.current_top = parseFloat(
                  ((images_config.preview_height - images_config.current_height) / 2).toFixed(2)
                )
                css_config = {
                      ...
                      event: function (row, index, ev, key, that) {
                        bt.confirm(
                          ...
                          function () {
                            ...
                            bt_tools.send(
                              { url: '/files?action=Re_Recycle_bin', data: { path: row.rname } },
                              function (ress) {
                                layer.msg(ress.msg, {
                                  icon: ress.status ? 1 : 2,
                                })
              ...
              tootls: [
                {
                  ...
                  selectList: [
                            ...
                            bt.input_confirm(
                              {
                                title: '批量删除数据库',
                                value: '删除数据库',
                                msg: '批量删除选中的数据库后，<span class="color-org">数据库将彻底删除，不可恢复</span>，是否继续操作？',
                              },
                              function () {
                                that.start_batch({}, function (list) {
                                  var html = ''
                                  ...
                                  database_bin_table.$batch_success_table({
                                    title: '批量删除数据库',
                                    th: '数据库名称',
                                    html: html,
                                  })
                    ...
                    },
                    {
                      ...
                      callback: function (that) {
                        bt.confirm(
                          { title: '批量恢复数据库', msg: '是否批量恢复选中的数据库，是否继续？', icon: 0 },
                          function (index) {
                            layer.close(index)
                            that.start_batch({}, function (list) {
                              var html = ''
    ...
    bt.input_confirm(
      {
        ...
        msg:
          '<span class="color-org">风险操作，删除选中的' +
          (row.type === 'folder' ? '文件夹' : '文件') +
          '将彻底消失</span>，此操作不可逆，是否继续操作？',
      },
      function () {
        var loadT = layer.msg(lan.files.recycle_bin_del_the, { icon: 16, time: 0, shade: [0.3, '#000'] })
        bt.send('Del_Recycle_bin', 'files/Del_Recycle_bin', { path: row.rname }, function (rdata) {
          layer.close(loadT)
          layer.msg(rdata.msg, { icon: rdata.status ? 1 : 5 })
          if (callback) callback(rdata)
        })
              ...
              bt_tools.send(
                {
                  ...
                  data: { send: 1, qq_mail: _email, qq_stmp_pwd: _passW, hosts: _server, port: _port },
                },
                function (configM) {
                  if (configM.status) {
                    layer.close(indexs)
  ...
  layer.open({
    ...
    success: function () {
      ...
      $('.btn-send-test').click(function () {
        bt_tools.send(
          {
            url: '/config?action=get_msg_fun',
            data: { module_name: 'wx_account', fun_name: 'push_data', msg: '发送测试信息' },
          },
          function (res) {
            if (res.status) {
