var bt = {
   ...
   */
  newDotTips: function (obj) {
    var storageData = bt.get_storage('newDotTips') // 获取本地存储
    storageData = storageData ? JSON.parse(storageData) : bt.newDotTipsData
    if (storageData[obj.name] && storageData[obj.name].status) {
      $(obj.el + ' .bt-red-dot').remove()
      //增加红点
      $(obj.el).css('position', 'relative')
      $(obj.el).append(
        '<div class="bt-red-dot inlineBlock ml6 red_dot_' +
          storageData[obj.name].type +
          '" style="position: absolute;background: #FF5050;' +
          (obj.style || '') +
          '"></div>'
      )
      $(obj.el).click(function () {
        $(obj.el + ' .bt-red-dot').remove()
        bt.newDotTipsData[obj.name].status = false
        bt.set_storage('newDotTips', JSON.stringify(bt.newDotTipsData))
        // 统计
      ...
      content:
        ...
        "</p>\
						<div class='confirm-info-box'>\
							<input onpaste='return false;' id='prompt_input_box' autocomplete='off' type='text' value=''>\
    ...
    if (bt.os == 'Linux' && sType === 0) {
      ...
      $.post(url, data, function (rdata) {
        ...
        if (typeof rdata == 'string') {
          if (
            (rdata.indexOf('/static/favicon.ico') != -1 && rdata.indexOf('/static/img/qrCode.png') != -1) ||
      ...
      for (var x = 0; x < item.items.length; x++) {
        ...
        switch (_obj.type) {
          ...
          case 'textarea':
            ...
            if (_placeholder)
              _html +=
                '<div class="placeholder c9" style="top: 15px; left: 15px; display: block;">' + _placeholder + '</div>'
            break
          case 'button':
            var _width = _obj.width ? _obj.width : '330px'
            _html +=
  ...
  render_form: function (data, callback) {
    if (data) {
      ...
      for (var i = 0; i < data.btns.length; i++) {
        ...
        _btn_html +=
          "<button type='button' class='btn btn-sm " + css + ' ' + item.name + bs + "' >" + item.title + '</button>'
        clicks.push({
          bind: item.name + bs,
          callback: item.callback,
        })
        ...
        case 'BUTTON':
          if (btn.prop('tagName') == 'BUTTON' || btn.attr('type') == 'checkbox') {
            btn.click(function () {
              ...
              var parent =
                $(this).parents('.bt-form').length === 0 ? $(this).parents('.bt-w-con') : $(this).parents('.bt-form')
    ...
    if (obj.columns) {
      ...
      for (var h = 0; h < obj.columns.length; h++) {
        ...
        if (item) {
          ...
          if (item.help)
            thead += '<a href="' + item.help + '" class="bt-ico-ask" target="_blank" title="点击查看说明">?</a>'
      ...
      if (obj.data.length > 0) {
        for (var i = 0; i < obj.data.length; i++) {
          ...
          for (var h = 0; h < obj.columns.length; h++) {
            ...
            if (item) {
              ...
              if (item.type == 'checkbox')
                _val =
                  '<input value=' + val[item.field] + '  class="check" onclick="bt.check_select();" type="checkbox">'
              var td = '<td '
              if (item.align) {
                td += 'style="'
        ...
        _tab.append("<tr><td colspan='" + obj.columns.length + "' class=>" + lan.bt.no_data + '</td></tr>')
  ...
  editor: function (config) {
    ...
    return layer.open({
      ...
      content:
        ...
        random +
        '" style="border:1px solid #ececec; height: ' +
        (config.height || '500px') +
        ';"></div>' +
        '</div>',
  ...
  // 保存编辑器文件
  saveEditor: function (ace, encode) {
...
bt.pub = {
  ...
  set_ftp_logs: function (type) {
    ...
    layer.confirm(
      typeName +
        'pure-ftpd日志管理后，' +
        (status ? '将记录所有FTP用户的登录、操作记录' : '将无法记录所有FTP用户的登录、操作记录') +
        '，是否继续操作？',
      {
...
bt.files = {
  ...
  dir_details_span: function (path, is_a) {
    var html = '<span class="dir_details_span">',
      _html = ''
    if ($('.file_list_content.file_manage').length) {
      html +=
        '<div class="dir_details" data-path="' +
        path +
        '">\
          ...
          bt.send(
            ...
            function (rdata1) {
              if (rdata1.setup && rdata1.endtime > 0) {
                bt_tools.send(
                  {
                    url: '/files/size/get_batch_path_size',
                    data: { path_list: JSON.stringify([path]), is_refresh: is_refresh ? is_refresh : false },
                  },
                  function (res) {
                    if (res['status'] === false) {
                      ...
                      var item =
                        res.data[path.lastIndexOf('/') == path.length - 1 ? path.substring(0, path.length - 1) : path]
                      ...
                      var html =
                        '<div class="details_cont_item"><span>目录详情<span>' +
                        stime +
                        '</span></span><button type="button" title="刷新" class="btn btn-default btn-sm details_refresh"><i class="icon-refresh"></i><span>刷新</span></button>\
                      ...
                      $('.authority')
                        ...
                        .click(function (e) {
                          ...
                          var arr = (
                            path.lastIndexOf('/') == path.length - 1 ? path.substring(0, path.length - 1) : path
                          ).split('/')
  ...
  get_files: function (Path, searchV, callback, sort) {
    ...
    bt.send(
      'get_files',
      'files/GetDir',
      'tojs=GetFiles&p=' + p + '&showRow=' + showRow + search + '&path=' + Path + sort_str,
      function (rdata) {
        loadT.close()
  ...
  del_recycle_bin: function (obj, callback) {
    bt.prompt_confirm(
      lan.files.recycle_bin_del_title,
      '您确定要删除文件[' + obj.name + ']吗，该操作将<span style="color:red;">永久删除该文件</span>，是否继续操作？',
      function () {
  ...
  clear_recycle_bin: function (callback) {
    bt.prompt_confirm(
      lan.files.recycle_bin_close,
      '您确定要清空回收站吗，该操作将<span style="color:red;">永久删除文件</span>，是否继续操作？',
      function () {
  ...
  un_zip: function (
    ...
    if (type.length == 3) {
      ...
      bt.send(
        'UnZip',
        'files/UnZip',
        'sfile=' + sfile + '&dfile=' + dfile + '&type=' + type + '&coding=' + coding + '&password=' + password,
        function (rdata) {
          loading.close()
          bt.msg(rdata)
          if (callback) callback(rdata)
...
bt.soft = {
  ...
  product_pay_view: function (config) {
    ...
    // 	return false;
    // }
    if (config.ps !== undefined)
      config.ps =
        format_introduce(config.ex1) ||
        config.ps.replace(/#1/g, '<').replace(/#2/g, '>').replace(/#3/g, '"').replace(/#4/g, "'")
    var that = this
    ...
    layer.open({
      ...
      content:
        ...
        ' ' +
        ((typeof config.closePro != 'undefined' && config.closePro) || config['type'] === 12 || config['is_alone']
          ? 'closePro'
          : '') +
        '">\
									' +
        (config.plugin
          ? '<div class="libPay-menu-type lib_plugin"><p>' + config.name + '</p><p>' + config.name + '</p></div>'
          : '') +
        '\
        ...
        function get_coupons() {
          bt.send('get_coupons', 'auth/get_coupons', {}, function (res_coupons) {
            ...
            bt.send('get_plugin_remarks', 'auth/get_plugin_remarks', {}, function (res) {
              ...
              $('.libPay-line-item.artificial')
                .unbind('click')
                .click(function () {
                  if (!$(this).hasClass('active')) {
                    $(this).addClass('active')
                    $('.libPay-menu .libPay-menu-type.active').data(
                      'data',
                      arry.filter(function (item) {
                        return item.pid === 100000068
                      })[0] || ops_obj
                    )
                  } else {
                    $(this).removeClass('active')
              ...
              $('.libPay-menu .libPay-menu-type').click(function () {
                ...
                if ($(this).prop('class').indexOf('ltd') !== -1) {
                  ...
                  _item = arry.filter(function (item) {
                    return item.pid === 100000032
                  })[0]
                }
              ...
              function useCoupons() {
                ...
                if (res_coupons.success && _item.type !== 'ver') {
                  var all_coupons_data = res_coupons.res,
                    coupons_data = []
                  var _ul = ''
                  for (var i = 0; i < all_coupons_data.length; i++) {
                    if (
                      all_coupons_data[i].product_id === _item.pid ||
                      all_coupons_data[i].extra_product_ids.indexOf(_item.pid) != -1
                    ) {
                  ...
                  if (coupons_data.length) {
                    ...
                    $('.libPay-line-item.coupons')
                      .empty()
                      .append(
                        '<i class="coupon—checkbox"></i><input type="checkbox" class="coupon—checkbox-input" />\
								优惠券（' +
                          coupons_data.length +
                          '）\
								<div class="coupon_item"><span>' +
                          coupons_data[0].name +
                          '</span><ul class="hide">' +
                          _ul +
                          '</ul></div>\
								<div class="countdown_item hide"><div class="hour"></div>小时<div class="minute"></div>分<div class="second"></div>秒后失效</div>'
                      )
                      .data('data', coupons_data)
                    ...
                    if (!$.isEmptyObject(temporary_coupon) && !$('.exclusive_coupon_popup').length) {
                      if (!(use_coupon_id !== null && use_coupon_id.indexOf(temporary_coupon.id) > -1)) {
                        var result = arry.find(function (item, index) {
                          return item.pid === temporary_coupon.product_id
                        })
                        ...
                        bt.open({
                          ...
                          content:
                            '<div class="exclusive_coupon_title">恭喜你获得专属优惠劵</div>\
												<div class="exclusive_coupon_info">\
													<div class="exclusive_coupon_cont">\
														<div class="exclusive_coupon_cont_left">\
															<div class="coupon_discount_price"><span>￥</span><span>' +
                            parseInt(temporary_coupon.val2) +
                            '</span></div>\
															<div class="coupon_fully_able">满' +
                            parseInt(temporary_coupon.val1) +
                            '元可用</div>\
														</div>\
                          ...
                          success: function (layero, index) {
                            ...
                            $(layero)
                              ...
                              .css({ padding: '35px 40px', 'border-radius': '18px' })
                            $(layero).prev().css({ background: 'rgb(0, 0, 0)', opacity: '0.45' })
                            $(layero).before(
                              $(
                                '<img class="exclusive_coupon_light" src="/static/img/exclusive_coupon_light.svg" alt="" />'
                              ).css({
                    ...
                    $('.libPay-line-item.coupons').empty().append(
                      '<i class="coupon—checkbox"></i><input type="checkbox" class="coupon—checkbox-input" />\
								优惠券（0）\
                  ...
                  coupon_li.unbind('click').click(function (ev) {
                    ...
                    if (!$('.libPay-line-item.coupons').hasClass('disabled'))
                      $('.libPay-line-item.coupons').addClass('active')
                    ...
                    if (item) {
                      ...
                      if (parseFloat(_coupon_data.val2) > 1) {
                        ...
                        $('[data-type=1]')
                          .find('.pay-foo-price')
                          .html('低至' + (item.price / ((item.cycle / 12) * 365)).toFixed(2) + '元/天')
                      } else {
                        // 7天体验券
                        ...
                        $('[data-type=1]')
                          ...
                          .html(
                            '<span><div class="libPrice">￥<i>' +
                              parseFloat(_coupon_data.val1).toFixed(2) +
                              '</i></div>/7天体验</span><p>原价:' +
                              item.sprice +
                              '元</p>'
                          )
                        $('[data-type=1]')
                          .find('.pay-foo-price')
                          .html('低至' + parseFloat(parseFloat(_coupon_data.val1) / 7).toFixed(2) + '元/天')
                        for (var i = 0; i < coupons_data.length; i++) {
  ...
  set_pay_status_icon: function () {
    ...
    bt.send('get_pd', 'ajax/get_pd', {}, function (res) {
      ...
      if (!is_pay) {
        proHTML =
          '<span class="btltd-gray" onclick="bt.soft.updata_ltd()" style="margin-right:4px" title="点击升级到企业版">免费版</span>'
        proHTML_button =
        ...
        proHTML_index =
          ' <span class="bt' +
          advanced +
          '">到期时间：<span style="color: #fc6d26;font-weight: bold;margin-right:5px">' +
          (res[1] === 0
            ? '永久授权'
            : (res[1] === -2 || res[2] === -2 ? '已过期' : bt.format_data(res[_index], 'yyyy-MM-dd')) +
              '&nbsp;&nbsp;<a class="btlink" onclick="bt.soft.updata_pro()">续费</a>') +
          '</span>'
      }
      // 其他界面
    ...
    if (btype !== 'ver') {
      ...
      if (btype !== 'plugin') {
        ...
        $.each(_data['rlist'], function (index, item) {
          _list +=
            '<div class="pro-introduce"><span class="glyphicon glyphicon-ok"></span><span>' + item + '</span></div>'
        })
        _html =
          ...
          _li.data('data', item).click(function () {
            ...
            if ($(this).data('data').cycle === '12' && $('.libPay-menu-type.lib_ltd').hasClass('active')) {
              ...
              if (artificial.hasClass('active')) {
                memu_type.data(
                  'data',
                  arry.filter(function (items) {
                    return items.pid === 100000068
                  })[0]
                )
              } else {
                memu_type.data(
          ...
          for (var i = 0; i < nums.length; i++) {
            ...
            var _btn = $(
              '<button data-nums="' +
                ...
                count +
                '台</span>' +
                (discount < 1 ? '<em>' + discount * 10 + '折</em>' : '') +
                '</button>'
            )
            _btn.data('data', count).click(function () {
              ...
              if (num > 1) {
                $('.more_msg').remove()
                $('.libPay-line-item.proN').append(
                  '<div class="more_msg">购买多台授权会默认减1台用于本机授权，剩余的授权台数以抵扣券的方式发放到当前账号，购买后请切换至抵扣券查看</div>'
                )
              }
              $(this).addClass('active').siblings().removeClass('active')
              ...
              var _price = 0,
                arr_coupon = [] //可使用优惠券数组
              if (
                $('.pay-cycle-btns.active').data('data').cycle === '12' &&
                $('.libPay-menu-type.lib_ltd').hasClass('active')
              ) {
                var arry = $('.libPay-menu').data('arry')
                var _menu_type_data = arry.filter(function (item) {
                  return item.pid === 100000068
                })[0]
                ...
                if (_data_cache) {
                  ...
                  $('.artificial_price').text(
                    '￥' +
                      _data_cache['12'].nums.filter(function (item) {
                  ...
                  bt.soft.pro.get_product_discount_by(p_id, function (rdata) {
                    $('.artificial_price').text(
                      '￥' +
                        rdata['12'].nums.filter(function (item) {
                          return item.count === num
                        })[0].price
                    )
              ...
              if (!$('.libPay-line-item.artificial').hasClass('active'))
      ...
      $.each(_arry, function (index, oitem) {
        ...
        if ($.inArray(oitem.product_id, [100000030, 100000011]) >= 0) {
          p_html =
            '<span class="pro-font-icon"><span class="glyphicon glyphicon-vip" style="margin-right:5px"></span><span>' +
            oitem.name +
            '</span></span>'
        } else if ($.inArray(oitem.product_id, [100000046, 100000032]) >= 0) {
          p_html =
            '<span class="ltd-font-icon"><span class="glyphicon glyphicon-vip" style="margin-right:5px"></span><span>' +
            oitem.name +
            '</span></span>'
        }
  ...
  create_pay_code: function (idx) {
    ...
    var that = this,
      ...
      _coupon =
        $('.libPay-line-item.coupons').hasClass('active') && !$('.libPay-line-item.coupons').hasClass('disabled')
          ? $('.coupon_item').data('data')
          : ''
    ...
    that.pro.create_order(params, function (rdata) {
      ...
      if (
        ((idx != active_idx || _product.pid != active_product.pid) && active_product.type != 'ver') ||
        _nums != _cycle.nums[nums_idx].count
      ) {
      ...
      if (rdata.status === false) {
        ...
        bt.send('get_credits', 'auth/get_credits', { uid: rdata.data.uid }, function (res) {
          ...
          $('.libPay-content-box .pay-type-btn')
            .eq(bt.soft.pub.availableBalance >= cash_fee ? 2 : $('.pay-type-btn.active').index())
            .click()
        })
  ...
  privilege_contrast: function () {
    ...
    that.get_soft_list(undefined, undefined, undefined, function (res) {
      ...
      bt.open({
        ...
        shadeClose: false,
        content:
          '<div class="privilege_contrast_title">\
        ...
        success: function (layero, index) {
          $(layero)
            .find('.layui-layer-content')
            .css({ padding: '25px 40px 0 40px', 'line-height': '22px', color: '#666' })
  ...
  show_pay_code: function (idx, couponId) {
    ...
    $('.libPaycode-foo-txt .libPayTotal').html(
      '<span style="font-size:18px;margin-right:2px;">¥</span>' + parseFloat(_data.data.cash_fee / 100)
    )
    $('.libPayCycle')
      ...
      .append(
        '<i>' +
          _num +
          '台</i><span class="org_price">原价: ' +
          org_price +
          '元</span>' +
          (_coupon ? '<span class="discount_amount">已使用优惠券，立减' + parseFloat(_coupon.val2) + '元</span>' : '')
      )
      ...
      $('.libPay-content-box .pay-type-btn')
        ...
        .click(function () {
          ...
          $('.layui-layer-tips').css({
            left: $('.pay-type-btn:eq(0)').offset().left,
            top: $('.pay-type-btn:eq(0)').offset().top - 45,
          })
          return false
        })
          ...
          if (!$('.successful_purchase_popup').length) {
            that.get_soft_list(undefined, undefined, undefined, function (res) {
              ...
              bt.open({
                ...
                content:
                  '<div class="successful_purchase_title">\
                  ...
                  (ltd < 0 && time_name === 'ops'
                    ...
                    : '<div class="dividing_line"></div>\
										<span>' +
                      bt.format_data(res[time_name === 'ops' ? 'ltd' : time_name], 'yyyy-MM-dd') +
                      '到期</span>') +
                  '</div>' +
                  (config.num > 1
                    ? '<div>本机默认授权1台，剩余' +
                      (config.num - 1) +
                      '台以抵扣券的方式发放到当前账号，请切换至抵扣券查看</div>'
                    : '') +
                  '</div>\
  ...
  pubUpdataPlugin: function (dataRes, minVersion) {
    var length = dataRes.versions.filter(function (item) {
      return parseFloat(item.m_version + '.' + item.version) >= minVersion
    }).length
    if (length) {
      // 有可用版本时，更新插件列表
      pubUpdata(dataRes)
      ...
      // 无可用版本时，更新插件列表
      bt_tools.send(
        { url: '/plugin?action=get_soft_list', data: { p: 1, type: 0, tojs: '', force: 1 } },
        function (res_refresh) {
  ...
  show_plugin_info: function (data, is_beta, config) {
    ...
    if (data.update || bt.soft.softData == undefined) {
      this.get_soft_find(data.name, function (res) {
        set_data(res)
      })
    } else {
        ...
        layer.open({
          ...
          content:
            '<style>\
											...
											<div class="plugin_' +
            (title === '更新' ? 'update' : 'install') +
            '_back"></div>\
													<div class="install-header-title">\
														<div class="install-tips">\
															<img style="height:40px;margin-top: 6px;" alt="" src="/static/img/soft_ico/ico-' +
            (data.name.indexOf('php-') >= 0 ? 'php' : data.name) +
            '.png">\
																<div class="plugin-title" style="' +
            (type == 5 && title != '更新' ? 'margin-top: 11px;' : '') +
            '">\
																	<div class="plugin-title-text"><span>' +
            data.title +
            '</span><div class="plugin-tag">\
																		<div style="border:1px solid #E1E1E1;color:#9a9a9a;" class="plugin-tag-item ' +
            (item_type === 7 ? '' : 'hide') +
            '"><span>免费</span></div>\
																		<div style="border:1px solid #F9EE9E;color:#FFC82F;" class="plugin-tag-item ' +
            (item_type === 8 ? '' : 'hide') +
            '"><span>专</span></div>\
																		<div style="border:1px solid #E8D6B3;color:#A3782F;" class="plugin-tag-item ' +
            (item_type === 12 ? '' : 'hide') +
            '"><span>企</span></div>\
																	</div></div>\
																	<div class="item ' +
            ...
            (title == '更新'
              ? ''
              : '<span class="c9" style="margin:0 20px">|</span><span class="btlink history_version" style="font-weight: normal;">更新日志</span>') +
            '\
																	</div>\
																		<div class="version_message">\
																		<span class="' +
            (title == '更新' ? '' : 'hide') +
            '" style="font-weight:normal;">最新版本：<span class="isNewBeta">' +
            (is_beta ? '测试版' + info.m_version + '.' + info.version : new_version) +
            '</span></span>\
																		<span class="' +
            (config && !config['select_html'] ? '' : 'hide') +
            '" style="font-weight:normal;">当前版本：' +
            (typeof info != 'undefined' ? info.m_version + '.' + info.version : '') +
            '</span>\
																		</div>\
																</div>\
														</div>\
            ...
            (config && config['select_html'] ? '' : 'hide') +
            ' version_select">' +
            (config && config['select_html'] ? config.select_html : '') +
            '</div>\
															<button type="button" class="btn btn-default compile_install_button ' +
            (config && config['compile'] ? '' : 'hide') +
            '" ' +
            (data.dependnet.length ? 'disabled' : '') +
            '>编译安装</button>\
															<button type="button" class="btn btn-success  ' +
            ($.isEmptyObject(data.dependnet) || dependnet_flag ? 'install_plugin_button' : 'install_enviroment') +
            '"  style="display: flex;align-items: center;"><span class="update_plugin_button_icon" style="display:' +
            (title === '更新' ? '' : 'none') +
            '"></span>' +
            (config && type == 5 && title != '更新' ? '极速安装' : '立即' + title) +
            '</button>\
														</div>\
													</div>\
													<hr>\
													<div class="plugin-img-introduce ' +
            (img_arr.length ? '' : 'hide') +
            '">\
            ...
            (title == '更新' ? '' : 'hide') +
            '">' +
            (data.update_msg ? data.update_msg : '暂无内容') +
            '</div>\
													</div>\
													<div class="divtable bt_table mtb20 hide" style="padding: 0 45px;">\
															<table class="table table-hover ' +
            (type == 5 ? 'hide' : JSON.stringify(data.dependnet) === '{}' ? 'hide' : '') +
            '">\
          ...
          success: function (layers, indexs) {
            ...
            $('.global-set-left')
              .unbind('click')
              .on('click', '.global-set-tab li', function () {
            ...
            $('.plugin-img-introduce .global-set-right').click(function () {
              layer.open({
                ...
                area: ['1000px', '800px'],
                content:
                  '<div class="plugin-introduce-content">\
											<div class="btns">\
											...
											<div class="plugin-introduce-view">' +
                  $('.plugin-img-introduce .global-set-right').html() +
                  '</div></div>',
                btn: false,
                title: false,
                success: function (indexs) {
                  ...
                  // 轮播图经过事件
                  $('.plugin-introduce-content').hover(
                    function () {
            ...
            $('.history_version').on('click', function () {
              ...
              bt.send(
                ...
                function (res) {
                  ...
                  if (!res.length) {
                    _html_history =
                      '<div style="padding:20px;width:100%;height:100%;display:flex;justify-content:center;align-items:center;padding-bottom:20px">暂无版本数据</div>'
                  }
                  layer.open({
                    type: 1,
                    area: ['520px', '400px'],
            ...
            $('.install_plugin_button').on('click', function () {
              layer.closeAll()
              var setup_name = bt.get_cookie('is_install'),
                flag = true
              if (setup_name) {
                ...
                if (parseInt(data.type) === 11 || data.update) {
                  bt.show_confirm(
                    ...
                    '<span style="color:red">' +
                      (data.author === '宝塔'
                        ? '更新过程可能会导致服务中断,是否继续升级？<br>建议您在服务器负载闲时进行软件更新。'
                        : '安装第三方插件存在一定的安全风险，是否继续安装？') +
                      '</span>',
                    function () {
                      bt.soft.input_package(data.name, data.tmp_path, data)
                    }
            ...
            $('.install_enviroment').click(function () {
              ...
              if (data.dependnet.need.length)
                _html_need = '<span>缺少必要依赖</span>' + data.dependnet.need.join('、') + '，是否安装？'
              ...
              layer.open({
                ...
                btn: ['立即安装', '取消'],
                content:
                  '<div class="bt-form hint_confirm"><div class="hint_title"><i class="hint-confirm-icon"></i><div class="hint_con select-form">检测到当前插件' +
                  _html_need +
                  _html_selected +
                  '</div>        </div>      </div>',
                yes: function () {
                  ...
                  // 存在选择项
                  if (data.dependnet.selected.length) {
                    if (!$('.select-form input[type="radio"]:checked').attr('value'))
                      return bt.msg({ status: false, msg: '请选择安装项' })
                      ...
                      },
                      {
                        ...
                        event: function (ev) {
                          ...
                          if (versionForm.$get_form_value().version.split('.').length == 3) {
                            version =
                              versionForm.$get_form_value().version.split('.')[0] +
                              '.' +
                              versionForm.$get_form_value().version.split('.')[1]
      ...
      content:
        ...
        '</a></p>\
											...
											<ul class="help-info-text c7">\
													' +
        (data.update
          ? '<li>更新过程可能需要几分钟时间，请耐心等候!</li>'
          : '<li>安装过程可能需要几分钟时间，请耐心等候!</li><li>如果已存在此插件，将被替换!</li>') +
        '\
											</ul>\
									</div>',
              ...
              if ($('#DiskAnalysis').data('taskStatus')) {
                layer.confirm(
                  ...
                  function (indexs) {
                    ...
                    bt_tools.send(
                      {
                        url: '/plugin?action=a&name=disk_analysis&s=remove_task',
                        data: {
                          id: parseInt(taskId),
                        },
                      },
...
bt.database = {
  ...
  set_data_access: function (name) {
    var loading = bt.load(),
      param = {
        url: 'database/' + bt.data.db_tab_name + '/GetDatabaseAccess',
  ...
  check_single_db_status: function (sid, load, name, isDialog) {
    bt_tools.send(
      { url: '/database?action=CheckDatabaseStatus', data: { sid: sid } },
      function (ress) {
  ...
  check_single_other_status: function (sid, load, name, type, isDialog) {
    bt_tools.send(
      { url: '/database/' + type + '/CheckDatabaseStatus', data: { data: JSON.stringify({ sid: sid }) } },
      function (ress) {
        bt.database.set_status_dom(name, ress, isDialog)
      },
      { verify: false, load: load }
  ...
  set_status_dom: function (name, ress, isDialog) {
    $((isDialog ? '.layui-layer-content' : '.pull-right') + ' .db_status_row')
      .find(' .db_status_type')
      .text(name)
    $((isDialog ? '.layui-layer-content' : '.pull-right') + ' .db_status_row')
      .find(' .db_status_text')
  ...
  append_select_status: function (that, type) {
    if ($(that).find('option:selected').attr('value') == 'all') {
      ...
      $(that).parent().find('.db_status_row').remove()
      $(that).before(
        '<div class="db_status_row">\
		<span class="db_status_icon status_icon status_success"></span>\
		<span class="db_status_type">' +
          $(that).find('option:selected').text() +
          '</span>状态：\
		<span class="db_status_text">查询中...</span>\
		</div>'
      )
      bt.database.check_single_other_status(
        $(that).find('option:selected').attr('value'),
        false,
        $(that).find('option:selected').text(),
        type
      )
  ...
  add_database: function (cloudList, callback) {
    ...
    if (bt.data.db_tab_name == 'mysql') {
      ...
      $('.add_to_db select').change(function () {
        ...
        if (db_obj.value == '0') db_obj.name = '本地服务器'
        bt.database.check_single_db_status(
      ...
      bt.database.check_single_other_status(
        cloudList[0].value,
        false,
        cloudList[0].value == '0' ? '本地服务器' : cloudList[0].name,
        bt.data.db_tab_name,
        true
      )
  ...
  sync_database: function (sid, callback) {
    var loadT = bt.load(lan.database.sync_the),
      param = {
        url: 'database/' + bt.data.db_tab_name + '/SyncGetDatabases',
        data: { data: JSON.stringify({ sid: sid }) },
      }
  ...
  input_sql: function (fileName, dataName) {
    if (bt.data.db_tab_name === 'pgsql_manager') bt.data.db_tab_name = 'pgsql'
    bt.show_confirm(
      lan.database.input_title,
      '<span style="color:red;font-size:13px;">【' + dataName + '】' + lan.database.input_confirm + '</span>',
      function (index) {
        var loading = bt.load(lan.database.input_the),
          param = {
            url: 'database/' + bt.data.db_tab_name + '/InputSql',
            data: { data: JSON.stringify({ file: fileName, name: dataName }) },
          }
        if (bt.data.db_tab_name == 'mysql')
          param = { url: 'database?action=InputSql', data: { file: fileName, name: dataName } }
        bt_tools.send(param, function (rdata) {
          loading.close()
          bt.msg(rdata)
        })
      ...
      bt_tools.send(param, function (rdata) {
        ...
        bt_tools.open({
          ...
          yes: function (formD, indexs) {
            ...
            bt_tools.send(
              { url: 'database/mongodb/SetDatabaseAccess', data: { data: JSON.stringify(data) } },
              function (rdata) {
                load.close()
                bt_tools.msg('设置成功')
                layer.close(indexs)
...
bt.send('get_config', 'config/get_config', {}, function (rdata) {
  ...
  if (rdata.user_info && rdata.user_info.status) {
    ...
    $('.bind-user').html(
      '<a href="javascript:bt.pub.bind_btname();" class="btlink" style="margin-left: 5px;">' +
        rdata_data.username +
        '</a>'
    )
  } else {
...
bt.site = {
  ...
  set_site_security: function (id, name, fix, domains, status, return_rule, https_status, callback) {
  ...
  create_let: function (data, callback) {
    var loadT = layer.open({
      ...
      offset: '30%',
      content:
        "<pre style='margin-bottom: 0px;height:250px;text-align: left;background-color: #000;color: #fff;white-space: pre-wrap;' id='create_lst'>正在准备申请证书...</pre>",
      success: function (layers, index) {
...
bt.data = {
  ...
  database: {
    ...
    data_add: {
      ...
      yes: function () {
        $('[name=sid]').after(
          '<a class="btlink" onclick="layer.closeAll();db_public_fn.get_cloud_server_list()" style="margin-left: 10px;">管理远程服务器</a>'
        )
        if (bt.data.db_tab_name == 'mongodb') {
      ...
      btns: [
        bt.form.btn.close(),
        {
          ...
          callback: function (rdata, load) {
            ...
            var loading = bt.load(),
              param = {
                url: 'database/' + bt.data.db_tab_name + '/SetDatabaseAccess',
      ...
      btns: [
        ...
        },
        {
          ...
          callback: function (rdata, load, callback) {
            var loading = bt.load(),
              param = {
                url: 'database/' + bt.data.db_tab_name + '/ResDatabasePassword',
                data: { data: JSON.stringify(rdata) },
              }
          ...
          items: [
            {
              ...
              placeholder:
                '每行填写一个域名，默认为80端口<br>泛解析添加方法 *.domain.com<br>如另加端口格式为 www.domain.com:88',
            },
          ],
        },
        {
        ...
        layer.open({
          ...
          yes: function (indexs) {
            ...
            if (type === 'site' || type === 'ftp') {
              bt.public.modify_path_quota(
                { data: JSON.stringify({ size: quota_size, path: row.path }) },
                function (res) {
                  if (res.status) {
                    bt.msg(res)
                    layer.close(indexs)
              ...
              bt.public.modify_mysql_quota(
                { data: JSON.stringify({ size: quota_size, db_name: row.name }) },
                function (res) {
