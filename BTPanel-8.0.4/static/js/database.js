...
var database = {
  ...
  database_table_view: function (search) {
    ...
    database_table = bt_tools.table({
      ...
      beforeRequest: function () {
        ...
        param.search = $('#bt_database_table .pull-right .bt_search')
      ...
      column: [
        ...
        },
        {
          ...
          template: function (row) {
            ...
            return (
              '<span class="flex" style="width:100px" title="' +
              type_column +
              '"><span class="size_ellipsis" style="width: 0; flex: 1;">' +
              type_column +
              '</span></span>'
            )
          },
      ...
      tootls: [
        {
          ...
          list: [
            ...
            },
            {
              title: 'phpMyAdmin',
              event: function () {
                window.open('http://' + window.location.hostname + '/pma')
                return false
                ...
                bt.open({
                  ...
                  content:
                    ...
                    <ul class="help-info-text c7"><li>面板访问需要登录面板后，才能通过面板访问phpMyAdmin</li><li>若通过面板为phpMyAdmin开启了密码访问，请使用【通过公共访问】</li>' +
                    (isEnable
          ...
          selectList: [
            {
              ...
              callback: function (that) {
                ...
                bt.simple_confirm(
                  {
                    title: '批量同步数据库',
                    msg: '批量同步选中的数据库，同步过程中可能存在风险，请在闲置时间段同步数据库，是否继续操作？',
                  },
                  function () {
                    var param = {}
            ...
            },
            {
              ...
              url:
                bt.data.db_tab_name == 'mysql'
                  ...
                  : '/database/' + bt.data.db_tab_name + '/ToBackup',
              load: true,
              param: function (row) {
      ...
      success: function (config) {
        ...
        if ($('.database_type_select_filter').length == 0) {
          ...
          $('#bt_database_table .bt_search').before(
            '<select class="bt-input-text mr5 database_type_select_filter" style="width:110px" name="db_type_filter">' +
              _option +
              '</select>'
          )
        ...
        if ($('#bt_database_table .tootls_top .pull-left .bt-desired').length === 0) {
          $('#bt_database_table .tootls_top .pull-left').append(
    ...
    $('#bt_database_table .pull-right .bt_search')
      ...
      .focus(function () {
        ...
        if (_this.search_hi.search_history.length > 0) {
            ...
            .hover(
              function () {
                ...
                $(this)
                  .find('.delete_single')
                  .append(
                    '<img class="delete_img" style="width:8px;height:8px" src="/static/images/delete_single.png" />'
                  )
          ...
          $('#bt_database_table .pull-right .bt_search')
            .find('.list_box_body')
            .append('<span style="color:#999">暂无数据</span>')
        }
        $('.delete_single').click(function (event) {
          ...
          bt.confirm({ msg: '是否删除改记录？', title: '删除历史记录' }, function () {
            bt_tools.send(
              { url: '/panel/history/remove_search_history', data: { name: 'databases', key: 'get_list', val: names } },
              function (ress) {
                layer.close(cload)
    ...
    $('#all_delete').click(function () {
      ...
      bt.confirm({ msg: '是否清空所有历史记录？', title: '清空历史记录' }, function () {
        bt_tools.send(
          { url: '/panel/history/clear_search_history', data: { name: 'databases', key: 'get_list' } },
          function (ress) {
            layer.close(dload)
            layer.msg(ress.msg, { icon: ress.status ? 1 : 2 })
    ...
    if (bt.data.db_tab_name == 'mysql') {
      ...
      var web_tab = bt_tools.tab({
        ...
        list: [
              ...
              var backup_table = bt_tools.table({
                    ...
                    group: [
                      {
                        ...
                        event: function (row, index, ev, key, that) {
                          ...
                          if (
                            ...
                            layer.msg('暂不支持存储位置【' + cloud_list[type] + '】恢复功能', {
                              icon: 2,
                              closeBtn: 1,
                              time: 0,
                              shadeClose: true,
                                        ...
                                        $.post(
                                          ...
                                          function (task) {
                                            if (task.status == 1) {
                                              ...
                                              $('.db_export_txt2 p:eq(1)').html(
                                                '请稍等，正在恢复数据库 <img src="/static/img/ing.gif">'
                                              )
                                              bt.send(
                                                'InputSql',
                                                'database/InputSql',
                                              ...
                                              taskStatus = 1
                                              //更新下载进度
                                              $('.db_export_txt p:eq(1)').html(
                                                '正在下载文件:已下载 ' + task.used + '/' + ToSize(task.total)
                                              )
                                            }
                                          }
                                        )
                ...
                methods: {
                  ...
                  del_site_backup: function (config, callback) {
                    bt.confirm(
                      {
                        title: '删除数据库备份[' + config.addtime + ']',
                        msg: '删除选中数据库备份文件后，<span class="color-red">该数据库备份文件将永久消失</span>，是否继续操作？',
                      },
                      function () {
                        bt_tools.send(
                          'database/DelBackup',
                ...
                success: function () {
                  ...
                  bt.newDotTips({
                    el: '#bt_backup_table .tootls_top .pull-left button:eq(1)',
                    name: 'customBackup',
                    style: 'top: -3px;margin-left: 5px;',
                  })
                },
                tootls: [
                  {
                    ...
                    list: [
                      {
                        ...
                        event: function (ev, that) {
                          var allLoad = layer.msg('正在备份中,请稍后......', {
                            icon: 16,
                            shade: [0.3, '#000'],
                            time: 0,
                          })
                          bt_tools.send({ url: '/database?action=ToBackup', data: { id: id } }, function (ress) {
                        ...
                        event: function (ev, that) {
                          ...
                          bt_tools.open({
                                ...
                                form: [
                                  {
                                    group: {
                                      type: 'other',
                                      boxcontent:
                                        '<div><div><p>数据表</p></div><div id="dataBaseTableMysql"></div></div>',
                                    },
                                  },
                                  {
                                    formLabelWidth: '71px',
                            ...
                            yes: function (formD, indexs) {
                              ...
                              var mysql_Backup_load = layer.msg('正在备份中，请稍后...', {
                                icon: 16,
                                time: 0,
                                shade: [0.3, '#000'],
                              })
                              bt_tools.send(
                                {
                                  ...
                                  data: { storage_type: btn_Type, table_list: JSON.stringify(table_list), id: id },
                                },
                                function (ress) {
                                  backup_table.$refresh_table_list()
                                  layer.msg(ress.msg, {
                    ...
                    config: {
                      ...
                      callback: function (that) {
                        bt.confirm(
                          {
                            title: '批量删除数据库备份',
                            msg: '批量删除选中的数据库备份，<span class="color-red">备份文件将永久消失</span>，是否继续操作？',
                            icon: 0,
                          },
                          function (index) {
                            layer.close(index)
          ...
          {
            title:
              '<p style="height:16px;padding:0;border:none;margin-right:2px;background-color: transparent;vertical-align: sub;" class="firwall_place_of_attribution"></p>增量备份',
            name: 'incrementalBackup',
            content:
              "<div id='webedit-con-database' class='bt-w-con webedit-con pd15' style='margin-left:0;height:495px;'></div>",
            success: function () {
      ...
      if (bt.data.db_tab_name == 'mongodb') {
                  ...
                  group: [
                    ...
                    },
                    {
                      ...
                      event: function (row, index, ev, key, that) {
                        bt.simple_confirm(
                          {
                            title: '删除备份文件-' + row.addtime + '',
                            msg: '删除选中备份文件后，<span class="color-red">该备份文件将永久消失</span>，是否继续操作？',
                          },
                          function (index) {
                            bt_tools.send(
                              { url: '/database/mongodb/DelBackup', data: { data: JSON.stringify({ id: row.id }) } },
                              function (ress) {
              ...
              .click(function () {
                ...
                bt_tools.open({
                  ...
                  yes: function (formD, indexs) {
                    ...
                    var mgdb_Backup_load = layer.msg('正在备份中，请稍后...', {
                      icon: 16,
                      time: 0,
                      shade: [0.3, '#000'],
                    })
                    bt_tools.send(
                      {
                        url: '/database/mongodb/ToBackup',
                        data: { data: JSON.stringify({ file_type: btn_Type, collection_list: table_list, id: id }) },
                      },
                      function (ress) {
                        mgdbTableFh.$refresh_table_list()
                        layer.msg(ress.msg, {
        ...
        bt.pub.get_data('table=backup&search=' + id + '&type=1&tojs=database.database_detail', function (frdata) {
          loadT.close()
          frdata.page = frdata.page
            .replace(/'/g, '"')
            .replace(/database.database_detail\(/g, 'database.database_detail(' + id + ",'" + dataname + "',")
                        ...
                        },
                        {
                          ...
                          event: function (row, index, ev, key, that) {
                            bt.simple_confirm(
                              ...
                              function (index) {
                                bt_tools.send(
                                  { url: '/database/pgsql/DelBackup', data: { data: JSON.stringify({ id: row.id }) } },
                                  function (ress) {
                  ...
                  .click(function () {
                    ...
                    bt_tools.open({
                      ...
                      yes: function (formD, indexs) {
                        ...
                        var pgsql_Backup_load = layer.msg('正在备份中，请稍后...', {
                          icon: 16,
                          time: 0,
                          shade: [0.3, '#000'],
                        })
                        bt_tools.send(
                          {
                            url: '/database/pgsql/ToBackup',
                            data: { data: JSON.stringify({ storage_type: btn_Type, table_list: table_list, id: id }) },
                          },
                          function (ress) {
  ...
  upload_files: function (name) {
    ...
    bt_upload_file.open(
      path,
      is_pgsql ? '.sql' : '.sql,.zip,.bak,.gz',
      is_pgsql ? '请上传sql' : lan.database.input_up_type,
      function () {
  ...
  backupList: function (type, db) {
      ...
      bt_tools.send({ url: 'project/binlog/get_databases' }, function (res) {
        if (res.data.length == 0) {
          ...
          $.each(alldbname, function (index, item) {
            arry1.push({
              title: item.name + (item.cron_id == null || item.cron_id.length == 0 ? ' [无备份任务]' : ''),
              value: item.value,
            })
            tbObj[item.value] = []
            $.each(item.table_list, function (indexs, tb) {
              tbObj[item.value].push({
                title: tb.tb_name + (tb.cron_id == null || tb.cron_id.length == 0 ? '[无备份任务]' : ''),
                value: tb.value,
              })
            })
          })
    ...
    function delCrontab(param, callback) {
      bt.input_confirm(
        {
          title: '删除备份任务【' + param.name + '】',
          value: '删除备份任务',
          msg: "删除备份任务后，<span class='color-org'>备份数据将永久消失</span>，是否继续操作？",
        },
        function () {
          bt_tools.send(
            {
    ...
    function getAlarmSetting(value) {
      ...
      bt_tools.send(
        ...
        function (alarms) {
          ...
          $('.sn-alarm--channel').html(
            alarmType != -1
              ? $('.sn-alarm--list .sn-alarm--item.active').hasClass('disabled')
                ? '请选择告警方式'
                : sendChannel[alarmType].name
              : '请选择告警方式'
          )
          // 告警方式选择
          $('.sn-alarm--selectBox')
      ...
      var backup_databases = bt_tools.table({
        ...
        column: [
          { fid: 'db_name', title: '数据库名', type: 'text' },
          {
            ...
            template: function (row) {
              return (
                '<span style="display:inline-block;max-width:200px;white-space: nowrap; text-overflow: ellipsis; overflow: hidden;" title="' +
                row.tb_name +
                '">' +
                row.tb_name +
                '</span>'
          ...
          { fid: 'last_backup_time', title: '备份时间', width: 150, type: 'text' },
          {
            ...
            group: [
              ...
              },
              {
                title: '记录',
                event: function (row, index, ev, key, that) {
                  that.new_resume_download_backup({
                    gp_type: 0,
                    title: '恢复',
                    datab_name: row.db_name,
                    url: '/project/binlog/restore_time_database',
                    params: { cron_id: row.cron_id },
                  })
                },
              },
              {
                ...
                event: function (row, index, ev, key, that) {
                  ...
                  layer.open({
                    ...
                    yes: function (indexs) {
                      ...
                      var _where1 = $('input[name=where1]'),
                        _hour = $('input[name=hour]'),
                        _minute = $('input[name=minute]')
                      if (_where1.length > 0) {
                      ...
                      switch (formData.type) {
                        ...
                        case 'hour-n':
                          ...
                          formData.hour = ''
                          if (formData.minute <= 0 && formData.where1 <= 0)
                            return bt.msg({ status: false, msg: '小时、分钟不能同时小于1！' })
                          break
                        // 天/日默认最小为1
                      }
        ...
        methods: {
              ...
              success: function (el, indexs) {
                ...
                bt_tools.table({
                  ...
                  column: [
                    {
                      ...
                      template: function (row) {
                        return (
                          "<span style='display:inline-block;width:310px;overflow:hidden;white-space: nowrap;text-overflow: ellipsis;' title='" +
                          row.localhost +
                          "'>" +
                          row.name +
                          '</span>'
                      ...
                      group: [
                              ...
                              yes: function (indexs) {
                                ...
                                if (result == '我已知晓') {
                                  bt_tools.send(
                                    {
                                      url: '/project/binlog/restore_time_database',
                                      data: { cron_id: config.params.cron_id, node_time: row.addtime },
                                    },
                                    function (res) {
                                      if (res.status) {
                        ...
                        },
                        {
                          ...
                          event: function (row, index, ev, key, that) {
                            bt_tools.send(
                              {
                                url: '/project/binlog/export_time_database',
                                data: { cron_id: config.params.cron_id, node_time: row.addtime },
                              },
                              function (res) {
                                if (res.status) {
                                  if (res.data) {
                                    bt_tools.open({
                                      ...
                                      content:
                                        ...
                                        '">' +
                                        res.data.name +
                                        '</td><td><div style="float:right;"><span>' +
                                        bt.format_size(res.data.size, true, 2) +
                                        '</span></div></td>\
																			<td style="text-align: right;"><span style="width: 150px;"><a onclick="bt.pub.copy_pass(\'' +
                                        res.data.path +
                                        '\')" class="btlink group_1_srk5n" title="复制路径">复制路径</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href="/download?filename=' +
                                        res.data.path +
                                        '" class="btlink group_2_srk5n" title="下载">下载</a></span></td>\
																			</tr></tbody></table></div>\
																			</div>',
                                    })
                                  } else {
                  ...
                  success: function () {
                    ...
                    $('#showBackupTable').append(
                      '<span class="tips" style="color:#ef0808">恢复：会从最早【全量备份】开始恢复至选择的【增量备份】时间点</span>'
                    )
                  },
                })
              },
          ...
          del_database_backup: function (config, callback) {
            bt.prompt_confirm(
              '删除备份任务',
              '删除备份任务[' + config.name + ']，删除此任务会连备份数据一起删除,是否继续？',
              function () {
                bt_tools.send(
                  {
                    url: 'project/binlog/delete_mysql_binlog_setting',
                    data: { cron_id: config.cron_id, backup_id: config.backup_id, type: 'manager' },
                  },
                  function (rdata) {
                    if (callback) callback(rdata)
                  },
                ...
                event: function (ev, that) {
                  ...
                  layer.open({
                            ...
                            group: {
                              ...
                              change: function (formData, element, that) {
                                if (type == 1) {
                                  ...
                                  var html =
                                    '<span class="tbName" style="white-space: nowrap; text-overflow: ellipsis; overflow: hidden;">所有</span>'
                          ...
                          },
                          {
                            ...
                            group: {
                              ...
                              change: function (formData, element, that) {
                                ...
                                var html =
                                  '<span class="tbName" style="width:280px;white-space: nowrap; text-overflow: ellipsis; overflow: hidden;">' +
                                  value.join(',') +
                                  '</span>'
                    ...
                    yes: function (indexs) {
                      ...
                      //添加
                      bt_tools.send(
                        { url: 'project/binlog/add_mysql_increment_crontab', data: formData },
                        function (res) {
                          // bt_tools.msg(res);
                          if (res.status) {
                            layer.close(indexs)
      ...
      function (res) {
        if (res.status) {
          bt.open({
            ...
            skin: 'databaseBackup',
            shift: 0,
            content: "<div id='webedit-con-database' class='bt-w-con webedit-con pd15' style='margin-left:0;'></div>",
            success: function () {
        ...
        if ($('#DataInputList').length <= 0) {
          bt.open({
            ...
            <div class="pd15 bt_table">\
              <div class="clearfix">\
                <button class="btn btn-default btn-sm" onclick="database.upload_files(\'' +
              name +
              '\')">' +
              ...
              bt.render_help(
                $('.database-pos .tabs-item.active').data('type') === 'pgsql'
                  ? [
                      '仅支持sql',
                      bt.os != 'Linux'
                        ? lan.database.input_ps3.replace(/\/www.*\/database/, path)
            ...
            success: function () {
              if (mType == 'mongodb')
                $('.demo-class')
                  .find('.help-info-text')
                  .html(
                    '<li>仅支持zip、gz、json、csv、（.json.zip|.json.gz|.csv.zip|.csv.gz）</li><li>bson 格式需压缩成 zip|gz 压缩包结构：目录/目录/*.bson文件（目录名无要求）</li>'
                  )
            },
          })
        }
          ...
          bt.render({
            ...
            columns: [
              ...
              },
              {
                ...
                templet: function (item) {
                  return (
                    '<span><a class="btlink" herf="javascrpit:;" onclick="bt.database.input_sql(\'' +
                    bt.rtrim(rdata.PATH, '/') +
                    '/' +
                    item.name +
                    "','" +
                    name +
                    '\')">导入</a>  | <a class="btlink" herf="javascrpit:;" onclick="database.rm_input_file(\'' +
                    bt.rtrim(rdata.PATH, '/') +
                    '/' +
                    item.name +
                    "','" +
  ...
  del_database: function (wid, dbname, obj, callback) {
    ...
    var loadT = bt.load('正在检测数据库数据信息，请稍候...'),
      param = {
        url: 'database/' + bt.data.db_tab_name + '/check_del_data',
    ...
    bt_tools.send(param, function (res) {
      ...
      layer.open({
        ...
        content:
          ...
          (is_db_type
            ? '<span class="color-red">注意：远程数据库暂不支持数据库回收站，选中的数据库将彻底删除</span><br>'
            : '') +
          (!recycle_bin_db_open
            ? '<span class="color-red">风险操作：当前数据库回收站未开启，删除数据库将永久消失</span><br>'
            : '') +
        ...
        success: function (layers) {
          ...
          var rdata = res.data,
            ...
            t_icon =
              ' <span class="glyphicon glyphicon-info-sign" style="color: red;width:15px;height: 15px;;vertical-align: middle;"></span>'
          for (var j = 0; j < rdata.length; j++) {
            for (var i = 0; i < del_data.length; i++) {
              if (rdata[j].id == del_data[i].id) {
                ...
                if (rdata[j].total < 2048)
                  t_size = '注意事项：当前数据库不为空，可能为重要数据，请谨慎操作.\n数据库：' + database_size
            ...
            column: [
              { fid: 'name', title: '数据库名称' },
              {
                ...
                template: function (row) {
                  return (
                    '<span class="' +
                    (row.is_database_rule ? 'warning' : '') +
                    '" style="width: 110px;" title="' +
                    row.t_size +
                    '">' +
                    row.f_size +
                    (row.is_database_rule ? t_icon : '') +
                    '</span>'
                  )
                },
              },
              ...
              },
              {
                ...
                template: function (row) {
                  return (
                    ...
                    ' title="' +
                    (row.is_time_rule && row.total != 0
                      ? '重要：此数据库创建时间较早，可能为重要数据，请谨慎操作.'
                      : '') +
                    '时间：' +
                    row.database_time +
                    '">' +
              ...
              },
              {
                ...
                template: function (row, index, ev, _that) {
                  ...
                  return (
                    '<span style="width: 110px;" class="' +
                    (_html === '彻底删除' ? 'warning' + (row.db_type > 0 ? ' remote_database' : '') : '') +
                    '">' +
                    _html +
                    '</span>'
                  )
          ...
          setTimeout(function () {
            ...
            $(layers)
              .find('.check_layer_message .message_count')
              .html('<span style="color:red">请仔细阅读以上要删除信息，防止数据库被误删</span>')
        ...
        yes: function (indes, layers) {
          ...
          title =
            typeof dbname === 'function'
              ? '二次验证信息，批量删除数据库'
              : '二次验证信息，删除数据库 - [ ' + dbname + ' ]'
          if (type !== 'mysql') {
            tips =
              '<span class="color-red">当前数据库暂不支持数据库回收站，删除后将无法恢复</span>，此操作不可逆，是否继续操作？'
          } else {
            tips = is_db_type
          ...
          layer.open({
            ...
            content:
              ...
              (type === 'mysql' && !recycle_bin_db_open
                ? '<br>风险操作：当前数据库回收站未开启，删除数据库将永久消失！'
                : '') +
              "</div>\
                  <div class='confirm-info-box'>\
    ...
    database_table = bt_tools.table({
              ...
              success: function (res, list, that) {
                ...
                $.each(list, function (index, item) {
                  html +=
                    ...
                    item.name +
                    '</td><td><div style="float:right;"><span style="color:' +
                    (res.msg.indexOf('失败') == -1 ? '#20a53a' : 'red') +
                    '">' +
                    res.msg +
                    '</span></div></td></tr>'
                })
      ...
      success: function (config) {
        ...
        if ($('.sqlserver_type_select_filter').length == 0) {
          ...
          $('#bt_sqldatabase_table .bt_search').before(
            '<select class="bt-input-text mr5 sqlserver_type_select_filter" style="width:110px" name="db_type_filter">' +
              _option +
              '</select>'
          )
...
var mongodb = {
    ...
    database_table = bt_tools.table({
      ...
      success: function (config) {
        ...
        if ($('.mongodb_type_select_filter').length == 0) {
          ...
          $('#bt_mongodb_table .bt_search').before(
            '<select class="bt-input-text mr5 mongodb_type_select_filter" style="width:110px" name="db_type_filter">' +
              _option +
              '</select>'
          )
  ...
  mongo_tools: function (db_name, res) {
    ...
    bt_tools.send(
      { url: 'database/mongodb/GetInfo', data: { data: JSON.stringify({ db_name: db_name }) } },
      function (rdata) {
        layer.close(loadT)
        if (rdata.status === false) {
          layer.msg(rdata.msg, {
        ...
        for (var i = 0; i < tableList.length; i++) {
          ...
          tbody +=
            '<tr>\
													<td><span style="width:150px;"  title="' +
...
var redis = {
  ...
  database_table_view: function () {
    ...
    $('#bt_redis_view .redis_cloud_server').html(
      '<span class="mr5" style="color:#f0ad4e"><span class="glyphicon glyphicon-info-sign"></span>当前所有操作项都关联至</span><select class="bt-input-text mr5 redis_type_select_filter" style="width:110px" name="db_type_filter">' +
        _option +
        '</select>'
    )
    $('.pull-right').css({ display: 'flex', 'align-items': 'center' })
    $('.redis_type_select_filter').after(
      '<div class="db_status_row">\
		<span class="db_status_icon status_icon status_success"></span>\
		状态：\
		<span class="db_status_text">正常</span>\
  ...
  render_redis_content: function (id) {
    ...
    bt.newDotTips({
      el: '#bt_redis_view .redis_content_view button:eq(3)',
      name: 'redisManag',
      style: 'top: -2px;margin-left: 6px;',
    })
    ...
    var tabHTML = ''
    bt_tools.send(
      { url: 'database/redis/get_list', data: { data: JSON.stringify({ sid: that.cloudInfo.sid }) } },
      function (rdata) {
      ...
      column: [
        ...
        { fid: 'name', title: '键', type: 'text' },
        {
          ...
          template: function (row) {
            ...
            return (
              '<div class="flex" style="width:350px" title="' +
              _val.text().replace(/\"/g, '&quot;') +
              '"><span class="size_ellipsis">' +
              _val.text() +
              '</span><span class="ico-copy cursor btcopy ml5" title="复制值"></span></div>'
            )
          },
                    ...
                    bt_tools.send(
                      {
                        ...
                        data: { data: JSON.stringify({ db_idx: id, key: row.name, sid: that.cloudInfo.sid }) },
                      },
                      function (rdata) {
                        if (rdata.status) {
      ...
      tootls: [
        ...
        },
        {
          ...
          config: {
            ...
            callback: function (that) {
              bt.confirm(
                { title: '批量删除key值', msg: '是否批量删除选中的key值，是否继续？', icon: 0 },
                function (index) {
                  layer.close(index)
                  that.start_batch({}, function (list) {
                    var html = ''
          ...
          column: [
            {
              ...
              template: function (item) {
                return (
                  '<span class="flex" style="width:154px" title="' +
                  item.name +
                  '"><span class="size_ellipsis" style="width: 0; flex: 1;">' +
                  item.name +
                  '</span></span>'
            ...
            },
            {
              ...
              template: function (item) {
                return (
                  '<span class="flex" style="width:280px" title="' +
                  item.filepath +
                  '"><span class="size_ellipsis" style="width: 0; flex: 1;">' +
                  item.filepath +
                  '</span></span>'
              ...
              group: [
                {
                  ...
                  event: function (row) {
                    bt.prompt_confirm('覆盖数据', '即将使用【' + row.name + '】对数据进行覆盖，是否继续?', function () {
                      bt_tools.send(
                        {
                          url: 'database/redis/InputSql',
                          data: { data: JSON.stringify({ file: row.filepath, sid: 0 }) },
                        },
                        function (rdata) {
      ...
      yes: function (index, layers) {
        ...
        layer.confirm(
          ...
          function (index) {
            bt_tools.send(
              {
                url: 'database/redis/clear_flushdb',
                data: { data: JSON.stringify({ ids: JSON.stringify(redisIDList), sid: that.cloudInfo.sid }) },
              },
              function (rdata) {
                if (rdata.status) {
                  that.render_redis_content()
  ...
  cluster_manage_viwe: function () {
    ...
    $('#cluster_manage_table').on('click', '.clusterServiceStatus', function () {
      ...
      bt_tools.send(
        { url: 'database/rediscluster/' + (status ? 'stop_cluster' : 'restart_cluster'), data: {} },
        function (res) {
          if (res.status) {
            rthat.get_cluster_list()
          }
  ...
  get_cluster_list: function () {
    var rthat = this
    bt_tools.send(
      { url: 'database/rediscluster/get_cluster_info', data: {} },
      function (clist) {
            ...
            if (fixedThead) {
              ...
              if (num >= 2) {
                ...
                if (btnPositionBox.find('.cluster_mode_switch').length == 0) {
                  btnPositionBox.append(
                    '<div style="display:inline">当前模式：【<span class="cluster_mode_title">无</span>】</div>'
                  )
                  $('#cluster_manage_table').find('.tootls_group.tootls_top .pull-right').append(
                    '<div class="cluster_mode_switch" style="display: inline-block;margin-left: 20px;">\
								<span>集群开关</span>\
    ...
    var array = [
      ...
      },
      {
        ...
        event: function (row, index, ev, key, that) {
          bt_tools.send(
            {
              url: 'database/rediscluster/set_container',
              data: { data: JSON.stringify({ node_id: row.node_id, status: row.status == '1' ? 'stop' : 'start' }) },
            },
            function (res) {
              if (res.status) {
            ...
            event: function (row, index, ev, key, that) {
              bt.confirm(
                ...
                function () {
                  bt_tools.send(
                    {
                      url: 'database/rediscluster/remove_cluster_node',
                      data: { data: JSON.stringify({ node_id: row.node_id }) },
                    },
                    function (res) {
                      if (res.status) {
                        redis.get_cluster_list()
  ...
  open_cluster_mode_config: function () {
    ...
    bt_tools.open({
        ...
        form: [
          ...
          },
          {
            group: {
              type: 'other',
              boxcontent:
                '<div class="compose_list_title">容器列表 【当前为：<span class="compose_mode_title"></span>】<button type="button" class="btn btn-success btn-xs ml15 add_redis_compose" style="vertical-align: initial;"><i class="glyphicon glyphicon-plus"></i> 添加容器</button></div><div class="redis_mode_table" id="redis_mode_table">\
      ...
      yes: function (formD, indexs) {
        ...
        if (formD.requirepass == '' || formD.requirepass.length < 5)
          return layer.msg('数据库密码不能小于5位', { icon: 0 })
        ...
        // 提交数据
        bt_tools.send(
          { url: 'database/rediscluster/set_cluster', data: { data: JSON.stringify(params) } },
          function (res) {
            if (res.status) {
              layer.close(indexs)
  ...
  render_cluster_mode_table: function () {
    ...
    $.each(arrData, function (index, item) {
      _body +=
        '<tr>\
				<td><input type="number" name="cluster_port_' +
        index +
        '" value="' +
        item.port +
        '"></td>\
				<td><input type="number" name="cluster_cpu_' +
        index +
        '" value="' +
        item.use_cpu +
        '"><span class="unit">个</span></td>\
				<td><input type="number" name="cluster_memory_' +
        index +
        '" value="' +
        item.use_memory +
        '"><span class="unit">MB</span></td>\
				<td><select name="cluster_role_' +
        index +
        '" value="' +
        item.role +
        '">\
...
var pgsql = {
    ...
    database_table = bt_tools.table({
          ...
          list: [
            {
              ...
              active: true,
              event: function () {
                if (cloudDatabaseList.length == 0)
                  return layer.msg('至少添加一个远程服务器或安装本地数据库', {
                    time: 0,
                    icon: 2,
                    closeBtn: 2,
                    shade: 0.3,
      ...
      success: function (config) {
        ...
        if ($('.pgsql_type_select_filter').length == 0) {
          ...
          $('#bt_pgsql_table .bt_search').before(
            '<select class="bt-input-text mr5 pgsql_type_select_filter" style="width:110px" name="db_type_filter">' +
              _option +
              '</select>'
          )
  ...
  pgsql_tools: function (db_name, res) {
    ...
    bt_tools.send(
      { url: 'database/pgsql/GetInfo', data: { data: JSON.stringify({ db_name: db_name }) } },
      function (rdata) {
        layer.close(loadT)
        if (rdata.status === false) {
          layer.msg(rdata.msg, {
        ...
        for (var i = 0; i < tableList.length; i++) {
          ...
          tbody +=
            '<tr>\
													<td><span style="width:250px;" title="' +
...
var db_public_fn = {
      ...
      success: function ($layer) {
        var tdHTML = [
          ...
          { fid: 'db_password', type: 'password', title: '管理员密码', copy: true, eye_open: true },
          {
            ...
            template: function (item) {
              return (
                '<span class="flex" style="width:144px" title="' +
                item.ps +
                '"><span class="size_ellipsis" style="width: 0; flex: 1;">' +
                item.ps +
                '</span></span>'
  ...
  render_cloud_server_table: function (callback) {
    var param = {
      url: 'database/' + bt.data.db_tab_name + '/GetCloudServer',
      data: { data: JSON.stringify({ type: bt.data.db_tab_name }) },
    }
    if (bt.data.db_tab_name == 'mysql')
      param = { url: 'database?action=GetCloudServer', data: { type: bt.data.db_tab_name } }
  ...
  del_db_cloud_server: function (row) {
    var that = this
    layer.confirm(
      '仅删除管理关系以及面板中的数据库记录，不会删除远程服务器中的数据',
      {
      ...
      function () {
        var param = {
          url: 'database/' + bt.data.db_tab_name + '/RemoveCloudServer',
        ...
        function (rdata) {
          if (rdata.task == '-1') {
            ...
            var pan =
              ...
              '</span>' +
              (rdata.status
                ? '<span class="glyphicon glyphicon-play ac"></span>'
            ...
            var manu = $(
              '<div class="flex manu" style="display:none;"><div class="arrow_b" style=""></div><div class="arrow" style=""></div></div>'
            )
            var status_list = [
              {
            ...
            for (var i = 0; i < status_list.length; i++)
              manu.append(
                '<div class="ac" onclick="bt.pub.set_server_status(\'' +
                  rdata.name +
                  "','" +
                  status_list[i].opt +
                  '\')">' +
  ...
  showDatabase: function (showTips) {
    ...
    if (bt.data.db_tab_name == 'mysql')
      requestParam = { url: 'database?action=GetCloudServer', data: { type: bt.data.db_tab_name } }
    bt_tools.send(requestParam, function (cloudData) {
      ...
      if (cloudData.length <= 0) {
        var tips =
          '当前未安装本地服务器/远程服务器,<a class="btlink install_server">点击安装</a> | <a class="btlink" onclick="db_public_fn.get_cloud_server_list()">添加远程服务器</a>'
        if (bt.data.db_tab_name == 'sqlserver')
          tips =
            '当前未配置远程服务器,<a class="btlink" onclick="db_public_fn.get_cloud_server_list()">添加远程服务器</a>'
