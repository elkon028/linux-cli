...
var index = {
  ...
  consultancy_services: function (show) {
    ...
    layer.open({
      ...
      btn2: function (indexs, layero) {
        layer.confirm(
          '是否放弃免费顾问服务，放弃后在遇到问题的时候可以在我们官网论坛发言，我们也有论坛值守人员协助您处理。',
          {
            title: '提示',
            area: '400px',
            btn: ['确认', '取消'],
  ...
  alarm_type_list: [
    { title: '【负载状态】告警', value: 'load' },
    { title: '【CPU使用率】告警', value: 'cpu' },
    { title: '【内存使用率】告警', value: 'mem' },
  ], // 告警任务类型
  msg_config: {}, // 告警通知配置
  release: false,
  interval: {
      ...
      obj.formatter = function (config) {
        ...
        $.each(list, function (index, item) {
          _tips +=
            ...
            (item.indexOf('time') > -1
              ? 'color:' + (data[item] > 100 && data[item] < 1000 ? '#ff9900' : data[item] >= 1000 ? 'red' : '#20a53a')
              : '') +
            '">' +
            options[item] +
            '：' +
            data[item] +
            (item.indexOf('time') > -1 ? ' ms' : ' 次/秒') +
            '</span><br />'
        })
        return _tips
      }
          ...
          group: [
            {
              ...
              event: function (row, index, ev, key, that) {
                ...
                if (row.important !== 1) {
                  bt.compute_confirm(
                    {
                      title: '结束【' + row.proc_name + '】进程',
                      msg:
                        '结束【' +
                        row.proc_name +
                        '】进程会导致' +
                        row.explain.split('的进程')[0] +
                        '无法正常运行，是否结束?',
                    },
                    function () {
                      load = bt.load()
  ...
  get_init: function () {
      ...
      bt_tools.send(
        ...
        function (res) {
          ...
          if (res.info && res.info.load_avg) {
            $('#load_one').text(
              res.info.load_avg['1'] + '  /  ' + res.info.load_avg['5'] + '  /  ' + res.info.load_avg['15']
            )
          }
          if (res.info) {
    ...
    var float_box = function (config) {
      ...
      if (is_other) {
        ...
        for (var i = 0; i < config.html.cpu_times.length; i++) {
          ...
          other_table_html +=
            ...
            (i + 1) +
            '_num">' +
            config.html.cpu_times[i] +
            '%' +
            (is_next ? ' / ' + config.html.cpu_times[i + 1] + '%' : '') +
            '</span>\
          </div>'
          i++
        }
      ...
      layer_id = layer.open({
        ...
        offset: [
          $(config.html.position).offset().top + 140 - $(window).scrollTop() + 'px',
          $(config.html.position).offset().left + 'px',
        ],
        content:
          '<div class=" load_tips disk_cont">\
            ...
            <div class="disk-item-list active base_html" id="base_info">\
              ...
              <div class="disk-item-body" id="base_info_html">' +
          config.html.base_html +
          '</div>\
              </div>' +
          (config.html.cpu_times
            ? '<div class="disk-item-list other_html ' +
              (is_other_active ? 'active' : '') +
              '" id="other_info">\
              <div class="disk-item-header">\
                <div class="disk-header-title">' +
              config.html.title +
              '</div>\
                <div class="disk-header-fold flex"><span class="mr8">' +
              (is_other_active ? '折叠' : '展开') +
              '</span><img src="/static/img/arrow-down.svg" ' +
              (is_other_active ? 'style="transform: rotate(180deg);"' : '') +
              ' class="icon-index-arrow"></div>\
              </div>\
              <div class="disk-item-body ' +
              (is_other ? 'cpu-core-body' : '') +
              '" id="other_info_html" ' +
              (is_other ? '' : 'style="padding: 16px 16px 0;') +
              '">' +
              (is_other
                ? other_table_html
                : '<table class="table" style="border:#ddd 1px solid;"><tbody>' +
                  other_table_html +
                  '</tbody></table>') +
              '</div></div>'
            : '') +
          ...
          (is_top_active ? 'active' : '') +
          '" id="top5_info">\
                    <div class="disk-item-header" mark="cpu">\
                      <div class="disk-header-title">' +
          config.html.disk_title +
          '</div>\
                  <div class="disk-header-fold flex"><span class="mr8">' +
          (is_top_active ? '折叠' : '展开') +
          '</span><img src="/static/img/arrow-down.svg" ' +
          (is_top_active ? 'style="transform: rotate(180deg);"' : '') +
          ' class="icon-index-arrow"></div>\
                </div>\
                <div class="disk-item-body disk-item-body-nottom" style="position:relative">\
                  <div class="disk-body-list no-bg">\
									<div class="load-list index_status_table" id="' +
          config.html.table_id +
          '"></div>\
                  \
        ...
        success: function (layero, index) {
          ...
          $(layero)
            ...
            .on('click', function () {
              ...
              var name =
                $(this).parents('.layui-layer-content').prop('id') + '_' + $(this).parents('.disk-item-list').prop('id')
    ...
    var cancel_float_box = function (dom_point, father_point) {
      ...
      postion_time = setInterval(function () {
        if ($(dom_point).length != 0) {
          ...
          var is_move_disk =
            mouseX >= diskChartX &&
            mouseX <= diskChartX + diskChartW &&
            mouseY >= diskChartY &&
            mouseY <= diskChartY + diskChartH
      ...
      bt_tools.send({ url: '/push?action=get_push_list' }, function (res) {
        _this.reander_system_info(function (rdata) {
          ...
          $('#loadChart').hover(
            function () {
              var loadCount =
                Math.round((rdata.load.one / rdata.load.max) * 100) > 100
                  ? 100
                  : Math.round((rdata.load.one / rdata.load.max) * 100)
              ...
              float_box({
                ...
                <div class="disk-body-list">\
                      ...
                      <span id="load_cpu_times_num">' +
                    rdata.cpu_times['活动进程数'] +
                    '  /  ' +
                    rdata.cpu_times['总进程数'] +
                    '</span>\
                    </div>\
                    <hr style="width: calc(100% - 16px);margin: 14px auto;">\
                    <div class="disk-body-alarm">\
                      <div class="flex align-center">告警\
                          <span class="bt_switch ml8" style="display:inline-block;"><input class="btswitch btswitch-ios" id="loadTips" type="checkbox" ' +
                    (_this.all_status.load['status'] ? 'checked' : '') +
                    '>\
                        ...
                        <a class="btlink set_alarm_status" data-type="load" href="javascript:;">设置</a>\
                          <span class=" c9 ml8 load_alarm_tips">5分钟内平均负载超过' +
                    (isLoad ? _this.all_status.load.count : '80') +
                    '%触发</span>\
                      </div>\
                </div>',
                  title: '更多负载信息',
              ...
              float_box({
                ...
                <div class="disk-body-list">\
                  <span>' +
                    ...
                    <div class="disk-body-alarm">\
                      <div class="flex align-center">告警\
                        <span class="bt_switch ml8" style="display:inline-block"><input class="btswitch btswitch-ios" id="cpuTips" type="checkbox" ' +
                    (_this.all_status.cpu['status'] ? 'checked' : '') +
                    '>\
                        ...
                        <span class="c9 ml8 cpu_alarm_tips">5分钟内平均CPU超过' +
                    (isCpu ? _this.all_status.cpu.count : '80') +
                    '%触发</span>\
                      </div>\
                </div>',
                  title: '核心使用率',
            ...
            function () {
              ...
              float_box({
                ...
                <div class="disk-body-list" title="由内核自动分配buffer/cache缓存大小，以提高文件系统性能和整体系统的响应速度">\
                      ...
                      <span id="mem_buffer_cache">' +
                    (rdata.mem.memBuffers + rdata.mem.memCached) +
                    'MB</span>\
                    </div>\
                    <hr style="width: calc(100% - 16px);margin: 14px auto;">\
                    <div class="disk-body-alarm">\
                      <div class="flex align-center">告警\
                        <span class="bt_switch ml8" style="display:inline-block"><input class="btswitch btswitch-ios" id="memTips" type="checkbox" ' +
                    (_this.all_status.mem['status'] ? 'checked' : '') +
                    '>\
                        ...
                        <span class="c9 ml8 mem_alarm_tips">5分钟内内存使用率超过' +
                    (isMem ? _this.all_status.mem.count : '80') +
                    '%触发</span>\
                      </div>\
                </div>',
                  slot:
                    ...
                    (((rdata.mem.memRealUsed / rdata.mem.memTotal) * 100).toFixed(2) >= 80
                      ? ((rdata.mem.memRealUsed / rdata.mem.memTotal) * 100).toFixed(2) >= 90
                        ? 'color-red-box'
                        : 'color-org-box'
                      : 'color-green-box') +
                    '">占用' +
          ...
          for (var i = 0; i < rdata.disk.length; i++) {
            if (_this.alarm_type_list.length == 3 + i)
              _this.alarm_type_list.push({ title: '磁盘【' + rdata.disk[i].path + '】告警', value: rdata.disk[i].path })
                  ...
                  if (!$('.disk_scanning_tips' + idx).length) {
                    ...
                    layer.open({
                      ...
                      content:
                        ...
                        '">\
                          <div class="disk_cont">\
                            <div class="disk-title">\
                                  <span>检测到当前磁盘<span class="' +
                        (used_size >= 80 ? (used_size >= 90 ? 'color-red-box' : 'color-org-box') : 'color-green-box') +
                        '">空间占用' +
                        used_size +
                        '%</span></span>\
                                <button type="button" data-path="' +
                        disk.path +
                        '" title="立即清理" class="btn btn-success disk_scanning_btn">立即清理</button></div>\
                            <div class="disk-item-list active">\
                              <div class="disk-item-header">\
                                <div class="disk-header-title">基础信息</div>\
                              </div>\
                              <div class="disk-item-body">\
                                  <div class="disk-body-list">类型：' +
                        disk.type +
                        '</div>\
                                  <div class="disk-body-list more_wrap"><div>挂载点：</div><div>' +
                        disk.path +
                        '</div></div>\
                                  <div class="disk-body-list use_disk" style="margin-top: 6px;white-space: pre-line;">可用：' +
                                    ...
                                    <div class="disk-body-alarm">\
                                      <div class="flex align-center">告警\
                                        <span class="bt_switch ml8" style="display:inline-block">\
                                          <input class="btswitch btswitch-ios" id="diskTips" type="checkbox" ' +
                        (_this.all_status.disk[disk.path] && _this.all_status.disk[disk.path]['status']
                          ? 'checked'
                          : '') +
                        '>\
                                        ...
                                        <div class="vertical-line"></div>\
                                        <a class="btlink set_alarm_status" data-spacesize="' +
                        parseFloat(disk.size[0]) +
                        '" data-type="disk" data-project="' +
                        disk.path +
                        '" href="javascript:;">设置</a>\
                                        <span class="c9 ml8 disk_alarm_tips">该磁盘' +
                        (isDisk
                          ? _this.all_status.disk[disk.path].cycle == 1
                            ? '余量不足' + _this.all_status.disk[disk.path].count + 'GB'
                            : '用量超过' + _this.all_status.disk[disk.path].count + '%'
                          : '空间占用超过80%') +
                        '触发</span>\
                                      </div>\
                                    </div>\
                        ...
                        (is_active ? 'style="transform: rotate(180deg);"' : '') +
                        ' class="icon-index-arrow">\
                                </div>\
															</div>\
                              <div class="disk-item-body">\
                                  <div class="disk-body-list more_wrap"><div>文件系统：</div><div>' +
                        disk.filesystem +
                        '</div></div>\
                                  <div class="disk-body-list inodes0">Inode总数：' +
                        disk.inodes[0] +
                        '</div>\
                                  <div class="disk-body-list inodes1">Inode已用：' +
                        disk.inodes[1] +
                        '</div>\
                                  <div class="disk-body-list inodes2">Inode可用：' +
                        disk.inodes[2] +
                        '</div>\
                                  <div class="disk-body-list inodes3">Inode使用率：' +
                        disk.inodes[3] +
                        '</div>\
                              </div>\
                            </div>\
                      ...
                      success: function (layero, index) {
                        ...
                        for (var key in _this.cache_system_push) {
                          ...
                          if (item.type === 'disk' && item.project === disk.path) {
                            ...
                            $(layero)
                              .find('.disk_alarm_tips')
                              .text(
                                '该磁盘' +
                                  (item.cycle == 1 ? '余量不足' + item.count + 'GB' : '用量超过' + item.count + '%') +
                                  '触发'
                              )
                          }
                        }
                          ...
                          .each(function (index, item) {
                            $(this).change(function () {
                              ...
                              if (!status) {
                                //存在告警关闭告警
                                bt.simple_confirm(
                                  { title: '关闭告警', msg: '关闭告警后，将不再接收到该项的告警信息，是否继续操作？' },
                                  function () {
                          ...
                          $(layero)
                            ...
                            .click(function () {
                              ...
                              if (type !== 'folder' && typeof type != 'undefined')
                                return layer.msg('文件暂不支持打开', { icon: 0 })
                              if (!taskList.hasOwnProperty(path.replace('//', '/'))) {
                                renderFilesView({
                                  path: root_path,
                                  subdir: path,
                  ...
                  diskInterval = setInterval(function () {
                    if ($('.disk_scanning_tips' + idx).length) {
                      ...
                      var is_move_disk =
                        ...
                        mouseY <= diskChartY + diskChartH
                      var is_move =
                        mouseX >= diskX && mouseX <= diskX + diskW && mouseY >= diskY && mouseY <= diskY + diskH
  ...
  open_alarm: function (obj, $check) {
    ...
    bt.open({
      ...
      btn: ['保存配置', '取消'],
      content:
        '<div class="bt-form pd16 index_alarm_set">\
                  <div class="line hide">\
                    <span class="tname">告警开关</span>\
                    <div class="info-r line-switch">\
                      <input type="checkbox" id="statusAlarm" class="btswitch btswitch-ios" name="status_alarm" ' +
        (isExpiration ? 'checked' : '') +
        ' />\
                  ...
                  <div class="line">\
                    ...
                    <div class="info-r">\
                      <div class="inlineBlock  ">\
                        <input type="text" name="type" disabled="disabled" class="bt-input-text mr10 " style="width:310px;" value="' +
        _this.alarm_type_list.filter(function (item) {
        ...
        '">\
                    ...
                    <div class="info-r">\
                      <div class="inlineBlock form-radio mr10">\
                        <input type="radio" id="cycle2" name="cycle" value="2" ' +
        (obj.cycle == 2 ? 'checked' : '') +
        '>\
                        <label for="cycle2" class="mb0">占用百分比</label>\
                      </div>\
                      <div class="inlineBlock form-radio">\
                        <input type="radio" id="cycle1" name="cycle" value="1" ' +
        (obj.cycle == 1 ? 'checked' : '') +
        '>\
                  ...
                  <div class="line">\
                    <span class="tname numTitle">' +
        numTitle +
        '</span>\
                    <div class="info-r">\
                      <div class="inlineBlock group" style="display: inline-flex;">\
                        <input type="number" name="count" min="0" class="bt-input-text mr10 group" style="width:70px;" value="' +
        (obj.hasOwnProperty('count') ? obj.count : 80) +
        '">\
                        <span class="unit">' +
        (isDisk && obj.cycle == 1 ? 'GB' : '%') +
        '</span>\
                      </div>\
                      <span class="type_tips ml8 c9">每' +
        (isDisk ? '隔10' : '5') +
        '分钟' +
        (isDisk ? (obj.cycle == 1 ? '磁盘剩余容量低于' : '磁盘占用超过') : '平均' + title + '超过') +
        '该数值就告警</span>\
                    </div>\
                  </div>\
                  <div class="line">\
                    ...
                    <div class="info-r">\
                      <div class="inlineBlock group" style="display: inline-flex;">\
                        <input type="number" name="push_count" min="0" class="bt-input-text mr10 group" style="width:70px;" value="' +
        (obj.hasOwnProperty('push_count') ? obj.push_count : 3) +
        '">\
      ...
      success: function (layero) {
        ...
        $('.index_alarm_set input[type="radio"]').change(function () {
          ...
          $('.type_tips').text(
            '每' +
              (isDisk ? '隔10' : '5') +
              '分钟' +
              (isDisk ? (val == 1 ? '磁盘剩余容量低于' : '磁盘占用超过') : '平均' + title + '超过') +
              '该数值就告警'
          )
        })
        $('.index_alarm_set input[type="number"]').on('input', function () {
          ...
          if ($(this).prop('name') == 'push_count') {
            ...
            if (val === '' || val <= 0 || (isDisk && cycle == 2 && val > 100) || (val > 100 && obj.type !== 'disk'))
              layer.tips(isDisk && cycle == 1 ? '数值不能小于0' : '数值范围(0,100]', $(this), {
                tips: [1, '#ff0000'],
                time: 3000,
              })
          }
        ...
        function renderConfigHTML(refresh) {
          if (refresh || $.isEmptyObject(_this.msg_config)) {
            //手动刷新 或者 全局配置_this.msg_config为空
            bt.site.get_msg_configs(function (rdata) {
              _this.msg_config = rdata
              pubConfig(rdata)
            })
          } else {
            //全局配置_this.msg_config不为空
      ...
      yes: function (index) {
        ...
        if (data.count === '' || data.count <= 0 || (data.count > 100 && obj.type === 'disk' && data.cycle == 2))
          return layer.msg(data.cycle == 1 ? '剩余容量范围不正确' : '百分比范围不正确', { icon: 2 })
        ...
        for (var key in _this.cache_system_push) {
          ...
          if (arr.indexOf(item.type) !== -1) {
            ...
            if (item.type === arr[3]) {
              ...
              $('.disk_scanning_btn[data-path="' + item.project + '"]')
                .parents('.disk_cont')
                .find('.disk_alarm_tips')
                .html(str)
            } else {
  ...
  get_server_info: function (info) {
    ...
    if (info.isuser > 0) {
      ...
      $('#messageError').append(
        '<p><span class="glyphicon glyphicon-alert" style="color: #ff4040; margin-right: 10px;"></span>' +
          ...
          '<span class="c7 mr5" title="此安全问题不可忽略，请尽快处理" style="cursor:no-drop"> [不可忽略]</span><a class="btlink" href="javascript:setUserName();"> [立即修改]</a></p>'
      )
    }
    ...
    if (info.isport === true) {
      $('#messageError').show()
      $('#messageError').append(
        '<p><span class="glyphicon glyphicon-alert" style="color: #ff4040; margin-right: 10px;"></span>当前面板使用的是默认端口[8888]，有安全隐患，请到面板设置中修改面板端口!<span class="c7 mr5" title="此安全问题不可忽略，请尽快处理" style="cursor:no-drop"> [不可忽略]</span><a class="btlink" href="/config"> [立即修改]</a></p>'
      )
    }
  ...
  reander_system_info: function (callback) {
    ...
    bt.system.get_net(function (res) {
      ...
      // 负载
      var loadCount =
        Math.round((res.load.one / res.load.max) * 100) > 100 ? 100 : Math.round((res.load.one / res.load.max) * 100)
      loadCount = loadCount < 0 ? 0 : loadCount
      var loadInfo = _this.chart_color_active(loadCount)
      //实时刷新负载弹窗数据
      $('.load_text')
        .text('运行达到' + loadCount + '%')
        .removeClass('color-red-box color-org-box color-green-box')
        .addClass(loadCount >= 80 ? (loadCount >= 90 ? 'color-red-box' : 'color-org-box') : 'color-green-box')
      ...
      $('.cpu_text')
        .text('占用' + cpuCount + '%')
        .removeClass('color-red-box color-org-box color-green-box')
        .addClass(cpuCount >= 80 ? (cpuCount >= 90 ? 'color-red-box' : 'color-org-box') : 'color-green-box')
      for (var i = 0; i < res.cpu[2].length; i++) {
        $('#cpu_' + (i + 1) + '_num').text(
          res.cpu[2][i] + '%' + (res.cpu[2].hasOwnProperty(i + 1) ? ' / ' + res.cpu[2][i + 1] + '%' : '')
        )
        i++
      }
      ...
      $('.mem_text')
        .text('占用' + memCount + '%')
        .removeClass('color-red-box color-org-box color-green-box')
        .addClass(memCount >= 80 ? (memCount >= 90 ? 'color-red-box' : 'color-org-box') : 'color-green-box')
      ...
      if (index.iostat.table)
        index.iostat.table.setOption({
          ...
          series: [
            ...
            { name: '写入字节数', data: index.iostat.data.dData },
          ],
        })
      //请求成功后，重新调用该方法
      if (res) {
  ...
  set_chart_data: function () {
    ...
    for (var i = 0; i < this.chart_view.disk.length; i++) {
      ...
      $('#diskName' + i)
        .text(this.chart_result.disk[i].path)
        .attr('title', this.chart_result.disk[i].path)
    }
  },
  /**
   * @description 赋值chart的数据
  ...
  get_index_list: function () {
    bt.soft.get_index_list(function (rdata) {
      ...
      // 推荐安装软件
      try {
        var recomConfig = false // product_recommend.get_recommend_type(1)
        if (recomConfig) {
          ...
          for (var i = 0; i < recomConfig['list'].length; i++) {
            ...
            softboxcon +=
              ...
              <div class="recommend-soft recom-iconfont" data-name="' +
              item['name'] +
              '">\
              ...
              (item['isBuy'] && !item['install']
                ? '<button class="btn btn-sm btn-success home_recommend_btn recommend_install" style="margin-left:0;" data-name="' +
                  item['name'] +
                  '">立即安装</button>'
                : '<a class="btn btn-sm btn-default mr5 ' +
                  ...
                  '" href="' +
                  item.preview +
                  '" target="_blank">预览</a><button type="submit" data-index="' +
                  i +
                  '" data-pay="' +
                  item.pay +
                  '" data-pluginType="' +
                  item.pluginType +
                  '" class="btn btn-sm btn-success home_recommend_btn pay_recommend_plungin">购买</button>') +
              '\
                </div>\
              </div>\
            </div>'
      ...
      if (bt.get_cookie('ltd_end') < 0) {
        $('.recommend-soft.recom-iconfont').hover(
          function () {
            var name = $(this).data('name')
            $(this)
              .find('.day-not-display')
              .fadeIn('1000')
              .find('span')
              .click(function (e) {
      ...
      $('.pay_recommend_plungin')
        .unbind('click')
        .click(function () {
          var item_pay = $(this).data('pay'),
            item_pluginType = $(this).data('pluginType'),
            index = $(this).data('index')
  ...
  check_update: function (state) {
    ...
    bt.system.check_update(function (rdata) {
      ...
      if (rdata.status) {
        ...
        <img class="update_bg" src="static/img/update_1.png" alt="" />' +
          ...
          '</div>\
            <div class="sub_title">' +
          '最新版本：' +
          '<a href="https://www.bt.cn/bbs/forum-36-1.html" target="_blank" class="btlink" title="查看版本更新日志">宝塔' +
          bt.os +
          versionType +
          ' ' +
          versionData.version +
          '</a>' +
          (!rdata.status ? '&nbsp;&nbsp;发布时间：' + versionData.uptime : '') +
          '</div>\
          </div>\
          ...
          '&nbsp;&nbsp;&nbsp;\
            ' +
          (!is_beta
            ? '<span>如需更新测试版请点击<a href="javascript:;" onclick="index.beta_msg()" class="btlink btn_update_testPanel">查看详情</a></span>'
            : '<span>如需切换回正式版请点击<a href="javascript:;" onclick="index.to_not_beta()" class="btlink btn_update_testPanel">切换到正式版</a></span>') +
          '\
            ' +
          (is_beta ? data.btb : '') +
          '\
        ...
        <div class="setchmod bt-form">\
          ...
          <div class="update_version"><span>当前版本：<a href="https://www.bt.cn/bbs/forum-36-1.html" target="_blank" class="btlink" title="查看当前版本日志">' +
          bt.os +
          (is_beta ? '测试版' : '正式版') +
          '&nbsp;&nbsp;' +
          versionData.version +
          '</a></span><span style="margin-left:25px">发布时间：' +
          versionData.uptime +
          '</span></div>' +
          '\
              <div class="update_conter">\
                  <div class="update_tips"><div class="tip_version"><span class="mr10">最新' +
          (is_beta ? '正式版' : '测试版') +
          '：' +
          (is_beta ? '正式版&nbsp;' + data.version : '测试版&nbsp;' + beta.version) +
          '</span><span class="ml10">更新时间：' +
          (is_beta ? data.uptime.replaceAll('/', '-') : beta.uptime.replaceAll('/', '-')) +
          '</span></div>\
                  ' +
          (!is_beta
            ? '<span style="margin-top:8px;">如需更新测试版请点击<a href="javascript:;" onclick="index.beta_msg()" class="btlink btn_update_testPanel">查看详情</a></span>'
            : '<span>如需切换回正式版请点击<a href="javascript:;" onclick="index.to_not_beta()" class="btlink btn_update_testPanel">切换到正式版</a></span>') +
          '\
                  ' +
          (is_beta
            ? '<span style="margin-top:8px;">每次升级测试版都会随机获得10-50个宝塔币奖励，可以进行<a href="https://www.bt.cn/bbs/thread-21014-1-1.html" rel="noreferrer noopener" target="_blank" class="btlink">礼品兑换</a></span>'
            : '') +
          '\
                  <span>有更新时提醒我：<span class="bt_switch" style="display:inline-block"><input class="btswitch btswitch-ios" id="updateTips" type="checkbox"><label class="btswitch-btn" for="updateTips"></label></span><a class="btlink setupdateconfig" style="margin-left: 15px;" href="javascript:;">提醒方式</a></span></span>\
                  </div>\
							...
							<div id="feedback">\
								...
								<span class="showNps" style="color:#20a53a;vertical-align:4px; font-size:12px;cursor:pointer;">点击此处折叠需求反馈<span class="glyphicon glyphicon-triangle-bottom ml5" style="display:' +
          (isNpsShow ? 'none' : 'inline') +
          ';"></span><span class="glyphicon glyphicon-triangle-top ml5" style="display:' +
          (isNpsShow ? 'inline' : 'none') +
          ';"></span></span>\
							</div>\
							<div class="NPSForm" style=" display:' +
          (isNpsShow ? 'block' : 'none') +
          ';">\
      ...
      bt.open({
        ...
        title: rdata.status
          ? false
          : ['宝塔' + bt.os + '面板', 'background-color:#37BC51;color:white;height:36px;padding:0 80px 0 13.5px;'],
        area: '550px',
        shadeClose: false,
        ...
        success: function (layers, indexs) {
              ...
              form: [
                ...
                },
                {
                  group: {
                    ...
                    type: 'other',
                    boxcontent:
                      '<div style="color:#20a53a;font-size:12px;position: relative;top: -12px;margin-left:39px;">我们特别重视您的需求反馈，我们会定期每周进行需求评审。希望能更好的帮到您</div>',
                  },
                },
                {
                  group: {
                      ...
                      bt_tools.send(
                        {
                          ...
                          data: { questions: JSON.stringify(config), software_name: 1, rate: 0, product_type: 18 },
                        },
                        function (ress) {
                          if (ress.status) {
                            layer.close(indexs)
                    ...
                    bt.site.get_msg_configs(function (rdata) {
                      ...
                      for (var key in rdata) {
                        ...
                        _html =
                          ...
                          '<div class="cursor-pointer form-checkbox-label mr10">' +
                          '<i class="form-checkbox cust—checkbox cursor-pointer mr5 ' +
                          (rdata1.module.indexOf(item.name) > -1
                            ? !item.setup || $.isEmptyObject(item.data)
                              ? ''
                              : key == 'wx_account' && !accountConfigStatus
                              ? ''
                          ...
                          item.title +
                          (!item.setup || $.isEmptyObject(item.data)
                            ? '[<a target="_blank" class="bterror installNotice" data-type="' +
                              item.name +
                              '">点击安装</a>]'
                            : key == 'wx_account' && !accountConfigStatus
                            ? '[<a target="_blank" class="bterror installNotice" data-type="' +
          ...
          function cacheModule(callback) {
            $.post('/push?action=get_push_config', { id: 'panel_update', name: 'site_push' }, function (rdata1) {
              if (typeof rdata1.code == 'undefined' && typeof rdata1.msg != 'undefined')
                return layer.msg(rdata1.msg, { icon: 2 })
  ...
  to_not_beta: function () {
    bt.simple_confirm(
      { title: '切换正式版', msg: '切换当前版本为正式版，切换后您将无法体验到最新的测试功能，是否继续操作？' },
      function () {
  ...
  re_server: function () {
    bt.open({
      ...
      shadeClose: false,
      content:
        '<div class="rebt-con"><div class="rebt-li"><a data-id="server" href="javascript:;">重启服务器</a></div><div class="rebt-li"><a data-id="panel" href="javascript:;">重启面板</a></div></div>',
    })
    setTimeout(function () {
            ...
            var rebootbox = bt.open({
              ...
              content:
                "<div class='bt-form bt-window-restart'>\
                  ...
                  <div class='warning_title'><div class='warning_icon'></div>" +
                ...
                "</p>\
                  <div class='SafeRestart'>\
                    <div class='info first_info'><span class='first_circle'></span><p>" +
                lan.index.reboot_ps_1 +
                "</p></div>\
                    <div class='info second_info'><span class='second_circle circle_gray'></span><p>" +
                lan.index.reboot_ps_2 +
                "</p></div>\
                    <div class='info third_info'><span class='third_circle circle_gray'></span><p>" +
                lan.index.reboot_ps_3 +
                "</p></div>\
                    <div class='info four_info'><span class='four_circle circle_gray'></span><p>" +
    ...
    bt.send('get_list', 'warning/get_list', obj, function (res) {
      if (res.status !== false) {
        ...
        $('.warning_scan_ps').html(
          that.warning_num > 0
            ? '本次扫描共检测到风险项<i>' + that.warning_num + '</i>个,请及时修复！'
            : '本次扫描检测无风险项，请继续保持！'
        )
        if (callback) callback(res)
      }
  ...
  getScanSpeed: function (sele, msg, url) {
    ...
    bt_tools.send({ url: url }, function (rdata) {
      body =
        '<p>正在' +
        msg +
        '，请稍后<img src="/static/img/ing.gif"></p>\
  ...
  reader_warning_view: function () {
    ...
    function reader_warning_list(data) {
      ...
      bt.each(arry, function (index, item) {
        ...
        html +=
          ...
          (item[index] == 'risk' && that.warning_num > 0
            ? '<i>点击折叠</i><span class="glyphicon glyphicon-menu-up" aria-hidden="false"></span>'
            : '<i>查看详情</i><span class="glyphicon glyphicon-menu-down" aria-hidden="false"></span>') +
          '</span>' +
          '</div>' +
          (function (index, item) {
            var htmls =
              '<ul class="module_details_list ' + (item[0] == 'risk' && that.warning_num > 0 ? 'active' : '') + '">'
            bt.each(data_item, function (indexs, items) {
              if (!items.cve_id) {
                ...
                htmls +=
                  ...
                  '，耗时：' +
                  (items.taking > 1 ? items.taking + '秒' : (items.taking * 1000).toFixed(2) + '毫秒') +
                  '&nbsp;，等级：' +
                  (function (level) {
                    var level_html = ''
                    switch (level) {
                  ...
                  (item[0] != 'security'
                    ? '<a href="javascript:;" class="btlink cut_details">详情</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href="javascript:;" data-model="' +
                      items.m_name +
                      '" data-title="' +
                      items.title +
                      '" ' +
                  ...
                  '<div class="module_details_block"><span class="line_title">检测类型：</span><span class="line_content">' +
                  items.title +
                  '</span></div>' +
                  ...
                  '</span></div>' +
                  '</div>' +
                  '<div class="module_details_line"><span class="line_title">风险描述：</span><span class="line_content">' +
                  items.msg +
                  '</span></div>' +
                  ...
                  '</span></div>' +
                  '<div class="module_details_line"><span class="line_title">温馨提示：</span><span class="line_content">' +
                  items.remind +
                  '</span></div>' +
                  (items.help != ''
                    ? '<div class="module_details_line"><span class="line_title">帮助文档：</span><span class="line_content"><a href="' +
                      items.help +
      ...
      $('.warning_scan_time').html(
        '检测时间：&nbsp;' +
          bt.format_data(scan_time) +
          '<a class="btlink" href="/project/security/report/html" target="_Blank" style="margin-left: 6px;text-decoration: underline;">>>获取检测报告 </a>'
      )
    }
              ...
              '<div class="dialog">\
                <div class="forbid">\
                    <div class="forbid_title">\
                        <div>\
                            <span>存在<span style="font-weight:600">' +
              dialog_forbid_data.length +
              '</span>项风险不可进行一键修复</span>\
							...
							<div class="forbid_content">\
								<p>\
									以上<span style="font-weight:600">' +
              dialog_forbid_data.length +
              '</span>项一键修复后可能会导致网站正常访问，服务运行异常等（请手动修复）\
								</p>\
							</div>\
						</div>\
						<div class="auto_content">\
							<div>\
								<span id="choose_auto_repair">请选择需要修复的风险项（已选中：<span id="danger_length" style="font-weight:600">' +
              0 +
              '</span>项）</span>\
            ...
            layer.open({
              ...
              success: function (layero, index) {
                  ...
                  column: [
                    ...
                    },
                    {
                      ...
                      template: function (row) {
                        return row.level == 1
                          ? '<span style="color:#e8d544">低危</span>'
                          : row.level == 2
                          ? '<span style="color:#E6A23C">中危</span>'
                ...
                if (!dialog_forbid_data.length) {
                  $('#choose_auto_repair').html(
                    '请选择需要修复的风险项（已选中：<span id="danger_length" style="font-weight:600">' +
                      0 +
                      '</span>项）</span>'
                  )
                ...
                thead =
                  '<thead><tr><th colspan="2">以下' + dialog_forbid_data.length + '项不可进行一键修复</th></tr></thead>'
  ...
  auto_repair: function (params, data) {
    ...
    $.post('/safe/security/set_security', { data: JSON.stringify({ m_name: params }) }, function (rdata) {
      ...
      var result_table =
        '<div><p style="margin-bottom:20px;font-size:16px">修复结果：修复成功<span style="color:#20a53a">' +
        rdata.success.length +
        '</span>项，修复失败<span style="color:red">' +
        rdata.failed.length +
        '</span>项，忽略<span style="color:#b9b9b9">' +
        rdata.cannot_automatically.length +
        '</span>项</p><div id="auto_repair_result"></div></div>' +
    ...
    bt.send('get_pd', 'ajax/get_pd', {}, function (res) {
      ...
      if (res[1] === 0) {
        $('.btpro span').click(function (e) {
          layer.confirm(
            '切换回免费版可通过解绑账号实现',
            { icon: 3, btn: ['解绑账号'], closeBtn: 2, title: '是否取消授权' },
            function () {
  ...
  recommend_paid_version: function () {
    try {
      ...
      if (recomConfig) {
        ...
        if (is_pay) {
          pay_html =
            '<div class="product-buy product-ltd ' +
            (advanced || item.name) +
            '-type">到期时间：<span>' +
            (end_time === 0
              ? '永久授权'
              : (end_time === -2 ? '已过期' : bt.format_data(end_time, 'yyyy-MM-dd')) +
          ...
          pay_html =
            '<div class="product-buy"><button type="button" onclick="product_recommend.pay_product_sign(\'' +
            (advanced || item.name) +
            "'," +
            item.pay +
            ',\'ltd\')"><span class="recommend-pay-icon"></span>立即升级</button></div>'
        }
        html =
          '<div class="conter-box bgw">\
          <div class="recommend-top radius4 ' +
          (is_pay ? advanced + '-bg recommend-ltd' : '') +
          '">' +
          (!is_pay ? pay_html : '') +
